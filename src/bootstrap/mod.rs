...
pub fn run() -> ! {
    ...
    {
        // The first slice proves only the modern transport lifecycle. Queue
        // ownership stays absent until descriptor/DMA/IRQ setup is complete.
        // SAFETY: the BSP window is mapped Device and no network owner exists.
        match unsafe { board::network::probe() } {
            Ok((base, result)) => println!(
                uart,
                "virtio-net: modern probe ok base={base:#x} vendor={:#x} features={:#x} reset",
