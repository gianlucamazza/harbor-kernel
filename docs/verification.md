...
| Product boot (`make product-boot-check`)                        | Boots the product image, asserts on the log                                  | Boot, console-server up, store agents loaded and ran, oracle strings absent, and the **invariant beacon** (`invariants: overwrites=0 abandoned=0 faults=0`) — the must-stay-zero set asserted in the shipped image                  | Nearly everything the oracle image asserts: ~9 assertions against ~100 — the subsystem invariants (reclaim, timer, refusals, faults) are exercised only in the oracle configuration                                                                                                                                                                              |
...
| ADR-0085            | K5 density residual policy                    | **Design/policy only** — no density code yet. Splits **K5-S** (Mini stack, first code ~0086), **K5-H** (driver multiplex, deferred), **K5-B** (pair collapse, trigger-gated). Thin (0044) stays done (HW). Forbids `MAX_TASKS++` as density win                                                                                                                                                          |
...
| ADR-0048            | K8 SMP design                                 | Design accepted; unpark paid [ADR-0070](adr/0070-k8-smp-first-slice.md); IPI paid [ADR-0074](adr/0074-k8-ipi-wake-second-slice.md); queues design [ADR-0075](adr/0075-k8-per-core-queues-design.md); code residual for multi-current + steal later                                                                                                                                                                  |
| ADR-0070            | K8 first slice (unpark core 1)                | `smp: core1 alive` — QEMU `raspi4b -smp 4` (spin-table PA `0xe0` + in-kernel table + PoC-clean root handoff). **Pi stamp 2026-08-09**, transcript `20260809-160348.log` (`cpu: Cortex-A72`, `CNTFRQ=54000000`, `hw-transcript-check` clean). Per-core queues: design ADR-0075; code residual                                                                                                                          |
