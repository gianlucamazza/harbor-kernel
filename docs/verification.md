...
The current capture is `.serial-log/20260815-092435.log` (one product
boot with a concurrent raw capture on the laptop Apple NIC);
`make hw-check` judges that boot. Boot (`src=19b6438d`, PowerOn)
prints
...
RX is timeout (no host frame). A system `tcpdump` on `enp0s20f0u5u2`
across that PowerOn (`.serial-log/20260815-103422-apple-nic.pcap`)
captured only the laptop's own DHCP broadcasts: no ethertype `0x88b5`
and no source `02:00:00:00:00:01`. Host RX counters stayed 0. Serial
complete is not a wire frame. None of those is a NIC.
