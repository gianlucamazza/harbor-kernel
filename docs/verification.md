...
The current capture is `.serial-log/20260814-140651.log` (four product boots);
`make hw-check` judges the last. Boot 4 (`src=61fe6774`) prints
`genet: binding ok base=0xfd580000 len=0x10000 phy=rgmii-rxid (fdt, not probed)`,
`genet: rev=6.0 patch=0x0 (mmio, not a nic)`, `loader: store n=5 image`,
...
Boots 2–3 (`src=92c889f4`) read the same SYS_REV_CTRL word and printed
`probe unavailable (Unsupported(6))` before the decoder accepted 6/7.
Boot 1 (`src=1aa3e894`) has the FDT line only. The earlier product
