...
| ADR-0077            | SMP shared-state discipline                   | Heap + sched + park under `IrqSpinLock`; single `arch::smp` resched; per-CPU mirrors. **F-R1-P1 complete (HW):** IPC, frames, ASID, naming, storage, taskcap, durable, console TX/RX model, IRQ wait/caps, MMU map/unmap (stamp 2026-08-10, `20260810-160227.log`). **Amended 2026-08-11:** loader `ENTRY_OF_TASK`/`ACTIVE` under `LOADER_LOCK` (missed after product `home_cpu` dual-core). Residual: lock refinement if measured; agent+TLB steal; coarse SCHED lock is N=2 only                                                                 |
