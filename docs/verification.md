...
**Whether that evidence is enough to accept ADR-0105 and ADR-0106 is the ADR
owner's judgement, not the driver's.** Until it is taken, they stay `proposed`,
`raspi4b` has no NIC backend, P3 stays `done (QEMU)`, and the product keeps
printing `authority: network vocabulary VACANT`.
