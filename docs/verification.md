...
in the *second* enable word), ring-slot sizing against `RX_BUF_LENGTH`, RDMA's
...
Transcripts are in `.serial-log/`; pcaps beside them. Every row is a PowerOn
boot of the product image unless noted, and `make hw-check` was clean on the
capture at the time it was taken.
...
It does not have a *published* one. The backend still sits below the EL1
