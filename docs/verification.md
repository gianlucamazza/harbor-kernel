...
11:17 is `src=b853980b`; earlier 10:23 is `src=a7891eb1`);
`make hw-check` judges the 11:17 PowerOn. That boot (`src=b853980b`,
PowerOn, CONS-without-UMAC) prints
...
`genet: desc ring enabled (dma, not a nic)`,
...
The first BMSR snapshot is down. TX printed complete (CONS posted).
The TSV window is all zero: packed `0x49c`, Linux `0x4a8`, `pok` `0x4ec`.
That is not a wrong-register artifact. RX is timeout (no host frame).
...
CONS, not a UniMAC send and not a wire frame. None of those is a NIC.
