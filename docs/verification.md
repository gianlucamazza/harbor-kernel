...
| ADR-0079            | K8 per-core timer + EL1 preempt on CPU 1      | `preempt-el1-cpu1: rotated` + `preempt-el1-cpu1: spinner exited` — local CNTP on affinity 1, global ticks CPU0-only, EL1 epilogue fence lifted (design ADR-0078). **done (QEMU)** via `boot-check`; HW stamp residual                                                                                                                                                                                          |
| ADR-0077            | SMP shared-state discipline                   | Heap + sched + park under `IrqSpinLock`; single `arch::smp` resched; per-CPU mirrors. Exercised on silicon with ADR-0076 stamp (`20260810-130305.log`). Residual: steal / per-core preempt / EL0-on-CPU1                                                                                                                                                                                                         |
| ADR-0078            | K8 per-core timer + preempt design            | **Design only** — no status flip. CPU0 owns global `ticks()`; each CPU programs CNTP + banked PPI 30; EL1 epilogue on CPU1 first (lift affinity fence); EL0-on-CPU1 out of scope. Code residual (follow-on ADR)                                                                                                                                                                                                 |
