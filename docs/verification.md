...
| ADR-0070            | K8 first slice (unpark core 1)                | `smp: core1 alive` — QEMU `raspi4b -smp 4` (spin-table PA `0xe0` + in-kernel table + PoC-clean root handoff). **Pi stamp 2026-08-09**, transcript `20260809-160348.log` (`cpu: Cortex-A72`, `CNTFRQ=54000000`, `hw-transcript-check` clean). No per-core runqueue yet (ADR-0048 residual)                                                                                                                                    |
...
| ADR-0072 + ADR-0073 | Hardware self-discovery (FDT report)          | `discover: model` / `memory` / `cpus` / `display` lines — host tests on fixture DTB; QEMU first boot with `-dtb` fixture, second boot DTB-less `unknown (no dtb)`; verify-don't-select (no map consumption). HW firmware-patched stamp residual                                                                                                    |
