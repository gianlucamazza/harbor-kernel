...
| Product boot (`make product-boot-check`)                        | Boots the product image, asserts on the log                                  | **Composition minimum** (~35 layered assertions): boot identity (MMU, reset, CPU, discovery, RNG), memory self-checks (frames, reclaim, unmap/split), IRQ seal + timer ticks, dual-current SMP (`core1 alive/ipi/ran`), console server + capability, multi-agent store (beacon+chirp load/run/wire), invariant beacon zeros, anomaly negatives, expanded oracle-string ban | Oracle demos (IPC refuse counts, EL0 session probes, density workers, peer transfer, …). Product still has fewer lines than `boot-oracle.sh`; that gap is intentional — product proves the **shipped path**, not every lab probe. Practice: CONTRIBUTING §5; knowledge `bp-verify-the-shipped-image`                                                                                                                                  |
| Oracle census (`make oracle-census`)                            | Compares `MAX_TASKS` across source, architecture table, and documented raise | Silent `MAX_TASKS++` without updating the capacity map; product peak occupancy vs ceiling ratio (ADR-0085: ceiling is oracle tax, not density)                                                                                                                                                       | Peak concurrency of the live oracle boot (call-site count only); whether a raise was *wise*. Practice: CONTRIBUTING §6; knowledge `bp-demo-ceiling-is-tax-not-density`                                                                                                                                                                                                                                                             |
...
| ADR                 | Slice                                         | Evidence                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ADR-0027 + ADR-0029 | K6 external store format + in-image placement | QEMU inject path in `make boot-check` (`loader: builtin` fallback when the store is empty); product store `n≥2` in `qemu-product-boot-check.sh`                                                                                                                                                                                                                                                                  |
| ADR-0088            | Product `home_cpu` / loader pin               | Host parse/pack `home_cpu`; `make product-boot-check`: `loader: beacon … home=0`, `loader: chirp … home=1`, both ran + wire bytes; sticky admit via `spawn_with_slots_on`                                                                                                                                                                                                                                                               |
...
| ADR-0090            | K10 force-exit Running                        | `force-kill: requested` / `child forced` / `slot empty` in `make boot-check` (EL1 Running + trampoline/agent `SessionEnd::Forced` path); host N/A                                                                                                                                                                                                                                                                                 |
...
| ADR-0093            | Panic path positive evidence                  | `make panic-check` — a `panic-probe` image writes to a real task stack's guard page after announcing its address. Twelve assertions: the probe ran and said where, the handler was reached from a *trap* (`sync exception EL1`), the printed `FAR` **equals the announced address**, the guard is named as a stack overflow, `KERNEL PANIC` appears once (re-entry guard held), `Harbor: hello` appears once (`cpu::halt` stopped the core), nothing follows `*** halt ***`. Seen red twice: announcing a different address than it writes fails on the `FAR` comparison; removing the store fails with "announced, never panicked" |
| ADR-0094            | Retire `debug-display`                        | The absence is the evidence: no `debug-display` in `Cargo.toml`, the `Makefile` or `src/`; `make check` green with one target fewer; the boot oracle now refuses **any** `display:` line, so a driver returning without a composition fails a gate rather than passing quietly. `kernel_core::{display, textgrid, font8x8, spi}` keep their host tests and their place in the mutation scope                                                                 |
