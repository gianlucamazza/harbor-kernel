...
The current capture is `.serial-log/20260814-232303.log` (eight product
boots with the laptop as link partner); `make hw-check` judges boot 8.
Boot 8 (`src=19b6438d`, PowerOn, TX CONS + CRC) prints
...
RX is timeout (no host frame). The laptop Apple NIC (`enp0s20f0u5u2`,
MAC `08:6d:41:e5:db:60`) received 0 frames. Serial complete is not a
