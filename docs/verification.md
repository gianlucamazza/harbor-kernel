...
13:27 is `src=807c9f52`; 13:07 is `src=5bf281a0`);
`make hw-check` judges the 13:27 PowerOn. That boot (`src=807c9f52`,
PowerOn, RING_CFG 0x1f) prints
...
The first BMSR snapshot is down. TX printed complete (CONS posted) for
the 124-byte TSB+probe after TDMA `RING_CFG=0x1f`. The TSV window is
...
Serial complete is DMA CONS, not a UniMAC send and not a wire frame.
TDMA `RING_CFG=0x1f` is not the missing send. None of those is a NIC.
