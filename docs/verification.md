...
The current capture is `.serial-log/20260814-232303.log` (one product boot
with the laptop as link partner); `make hw-check` judges it. Boot
(`src=30603cba`, PowerOn) prints
...
`link down` and not `complete`. The laptop Apple NIC (`enp0s20f0u7`)
received 0 frames. None of those is a completed frame or a NIC.
