...
The current capture is `.serial-log/20260814-232303.log` (three product
boots with the laptop as link partner); `make hw-check` judges boot 3.
Boot 3 (`src=3ee58791`, PowerOn, UniMAC speed slice) prints
...
NIC (now `enp0s20f0u5u2`, MAC `08:6d:41:e5:db:60`; earlier
`enp0s20f0u7`) received 0 frames. Speed programming did not put a
frame on the wire. None of those is a completed frame or a NIC.
