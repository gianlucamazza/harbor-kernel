...
| EL0 programs (in `make test`)             | Disassembles each `kernel_core::prog` encoder | That the bytes an agent runs are the instructions the doc-comment claims — the oracle is `llvm-mc`, so nobody transcribes hex in either direction | Whether the program is the *right* one for the test; only that it is the one written down |
...
| The disassembler is missing | shadow `llvm-mc` with a command that exits non-zero | `panicked: llvm-mc failed` — the test **fails** rather than skipping. `make no-simd` once reported `clean` having disassembled nothing, and that lesson is written into this helper's doc-comment |
...
| Orphaned trait behind a feature | remove the attribute from `SpiDevice` and build `--features debug-display` | `trait SpiDevice is never used`. It has an implementation (`ExclusiveDevice`) and no caller in any configuration, while ADR-0010 says short register ops go through `SpiDevice::write` — they go through `with_bus`. An accepted ADR and the code disagree; the trait keeps an `allow` with the reason attached until a successor ADR says which one is right |
