...
| ADR-0046            | K4 cooperative budget                         | `budget: rotated` (QEMU; Pi stamp 2026-08-08). EL1 workers only; the spinning-EL0-agent gap this row used to carry is closed by ADR-0064's IRQ-side enforcement                                                                                                                      |
...
| ADR-0064            | K4 EL0 preemption first slice                 | `preempt: rotated` + `preempt: spinner exited` (QEMU; **HW stamp pending** — the roadmap row stays done (QEMU) until the serial transcript). Host: `kernel_core::preempt` + `Switch::Preempt` model tests                                                                            |
