...
| EL0 programs (in `make test`)             | Assembles the intended text, compares bytes | That the bytes an agent runs are the instructions the doc-comment claims — `llvm-mc` assembles, so nobody transcribes hex in either direction | Whether the program is the *right* one for the test; only that it is the one written down |
