...
13:58 is `src=656be102`; 13:45 is `src=d1cfbca5`);
`make hw-check` judges the 13:58 PowerOn. That boot (`src=656be102`,
PowerOn, `RING_BUF_EN`) prints
...
The first BMSR snapshot is down. TX printed CONS retire for the
124-byte TSB+probe after TDMA `RING_BUF_EN=0x1f`. The TSV window is
all zero. RX is timeout. The Apple NIC pcap across that boot has no
`0x88b5` and no source `02:00:00:00:00:01` (only the laptop's DHCP).
`tx cons` is DMA CONS, not a UniMAC send and not a wire frame.
`RING_BUF_EN=0x1f` is not the missing send. None of those is a NIC.
