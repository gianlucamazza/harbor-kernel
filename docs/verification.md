...
| ADR-0077            | SMP shared-state discipline                   | Heap + sched + park under `IrqSpinLock`; single `arch::smp` resched; per-CPU mirrors. **F-R1-P1 complete (HW):** also IPC, frames, ASID, naming, storage, taskcap, durable, console TX/RX model, IRQ wait/caps, MMU map/unmap. Silicon stamp 2026-08-10, transcript `20260810-160227.log` (`src=05a38149`, Cortex-A72, `smp: steal ok` + el0-cpu1 + `hw-transcript-check` clean). Residual: lock refinement if measured; agent+TLB steal policy                                                                 |
