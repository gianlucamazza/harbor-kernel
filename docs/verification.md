...
13:07 is `src=5bf281a0`; 12:54 is `src=f0bae772`);
`make hw-check` judges the 13:07 PowerOn. That boot (`src=5bf281a0`,
PowerOn, TDMA WRR) prints
...
the 124-byte TSB+probe after TDMA `ARB_CTRL=DMA_ARBITER_WRR`. The TSV
window is all zero. RX is timeout. The Apple NIC pcap across that boot
has no `0x88b5` and no source `02:00:00:00:00:01` (only the laptop's
DHCP). Serial complete is DMA CONS, not a UniMAC send and not a wire
frame. TDMA WRR is not the missing send. None of those is a NIC.
