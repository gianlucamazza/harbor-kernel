...
| Product boot (`make product-boot-check`)                        | Boots the product image, asserts on the log (`scripts/lib/product-oracle.sh`, shared with `hw-transcript-check`)                                  | **Composition minimum** (~35 layered assertions): boot identity (MMU, reset, CPU, discovery, RNG), memory self-checks (frames, reclaim, unmap/split), IRQ seal + timer ticks, dual-current SMP (`core1 alive/ipi/ran`), console server + capability, multi-agent store (beacon+chirp load/run/wire), invariant beacon zeros, anomaly negatives, expanded oracle-string ban | Oracle demos (IPC refuse counts, EL0 session probes, density workers, peer transfer, …). Product still has fewer lines than `boot-oracle.sh`; that gap is intentional — product proves the **shipped path**, not every lab probe. Practice: CONTRIBUTING §5; knowledge `bp-verify-the-shipped-image`                                                                                                                                  |
