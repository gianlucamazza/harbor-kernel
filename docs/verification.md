...
The current capture is `.serial-log/20260816-052739.log` (reflash of
`src=a7891eb1`; prior two-boot capture `20260815-092435.log`);
`make hw-check` judges the 10:23 PowerOn. That boot (`src=a7891eb1`,
PowerOn, UniMAC datapath) prints
...
The UniMAC TX good-packet word at `0x4a8` is 0. RX is timeout (no
host frame). The Apple NIC pcap across that boot has no `0x88b5` and
no source `02:00:00:00:00:01` (only the laptop's DHCP). Serial
complete is DMA CONS, not a UniMAC send and not a wire frame. None of
those is a NIC.
