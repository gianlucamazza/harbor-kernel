...
| Debug builds (`make debug-builds`, `make debug-display-builds`) | Compiles and lints the dev profile and `--features debug-display`            | Those configurations no longer compiling                                                                                                                                                                                            | **They compile, they are not executed** — ~1.2k LoC of SPI/TFT/status code has no oracle line and no current HW stamp (the 2026-08-08 stamp ran headless). Stated here so the island is a known blind spot, not an implied covered layer                                                                                                                         |
...
| ADR-0080            | K8 EL0-on-CPU1 design                         | **Design only** — code [ADR-0081](adr/0081-k8-el0-on-cpu1-first-slice.md) **done (QEMU)**. Per-CPU `CURRENT_EL0`; publish on every affinity; sticky home; EL0 preempt on CPU1; steal still later                                                                                                                                                                                                              |
...
| ADR-0082            | K8 work-stealing design                       | **Design only** — code [ADR-0083](adr/0083-k8-work-stealing-first-slice.md) **done (QEMU)**. Hard re-home; pull-on-idle; opt-in stealeable (no agent AS without TLB IPI)                                                                                                                                                                                                      |
