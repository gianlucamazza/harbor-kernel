...
The current capture is `.serial-log/20260814-174803.log` (four product boots);
`make hw-check` judges the last. Boot 4 (`src=0b1dc0b9`) prints
...
`genet: queue0 enabled (dma, not a nic)` after `frames:` (invariants
...
Cable-down is `link=down`, not a failure. Enable is not a TX or RX.
A later image may print one `TxReport` after Enabled; that line is unpaid.
