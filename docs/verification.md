...
The current capture is `.serial-log/20260814-232303.log` (seven product
boots with the laptop as link partner); `make hw-check` judges boot 7.
Boot 7 (`src=fa00d083`, PowerOn, honest TX timeout) prints
...
The first BMSR snapshot is down. TX is `still owned` (CONS posted,
Device still OWNs), not `mdio timeout` and not `complete`. RX is
timeout (no host frame). The laptop Apple NIC (`enp0s20f0u5u2`, MAC
`08:6d:41:e5:db:60`) received 0 frames. CONS moved; no frame on the
wire. None of those is a completed frame or a NIC.
