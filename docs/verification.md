...
The current capture is `.serial-log/20260814-174803.log` (seven product boots);
`make hw-check` judges the last. Boot 7 (`src=30603cba`, PowerOn) prints
...
`genet: reset recovered (idle, not a nic)` after `frames:` (invariants
`frames_free=510`), `loader: store n=5 image`,
`authority: bound console`, `blob: put ok` / `blob: got`, and
`invariants: … slots=4/9`. Network stays `authority: network vocabulary VACANT`.
Encoded 6/7 are GENET v5 (Linux remaps them to logical 5); encoded 5 is v4.
Cable-down is `link=down`, not a failure. The TX and RX lines are refuses
before the doorbell / `RX_EN`; recover then returns Idle. None of those
is a completed frame or a NIC.

Boot 6 (`src=9cd383eb`) paid the RX refuse. Boot 5 (`src=34b3d132`)
paid the TX refuse. Boot 4 (`src=0b1dc0b9`) paid queue-0 enable.
Boot 3 (`src=f7404550`) paid queue-0 program. Boot 2 (`src=ee650e06`)
paid link classify. Boot 1 (`src=58b7448c`) paid PHY identify. The
earlier multi-boot capture `20260814-140651.log` paid FDT + `rev=6.0`.
The product capture `20260814-113438.log` (`src=dcc997cc`) paid
ADR-0102/0103.
