...
The current capture is `.serial-log/20260814-174803.log` (two product boots);
`make hw-check` judges the last. Boot 2 (`src=ee650e06`) prints
...
`genet: link=down (bmsr, not a nic)`, `loader: store n=5 image`,
...
Cable-down is `link=down`, not a failure.

Boot 1 in the same file (`src=58b7448c`) paid PHY identify without the
link line. The earlier multi-boot capture `20260814-140651.log` paid FDT
+ `rev=6.0`. The product capture `20260814-113438.log` (`src=dcc997cc`)
paid ADR-0102/0103.
