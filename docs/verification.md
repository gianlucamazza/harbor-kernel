...
| ADR-0076            | K8 per-core queues first slice                | `smp: core1 ran` — dual-current pure model, sched spinlock, `spawn_on(1)`, SGI resched, pinned marker (never exits — heap not multi-core-safe). QEMU oracle; HW stamp residual. Product still one schedulable core for demos after marker quiet-parks                                                                                                                                                           |
