...
The current capture is `.serial-log/20260814-174803.log` (six product boots);
`make hw-check` judges the last. Boot 6 (`src=9cd383eb`, PowerOn) prints
...
`genet: rx unavailable (link down)` after `frames:` (invariants
...
before the doorbell / `RX_EN`, not completed frames. A later image may
print one `ResetReport` after those refuses; that line is unpaid.

Boot 5 (`src=34b3d132`) paid the TX refuse. Boot 4 (`src=0b1dc0b9`)
paid queue-0 enable. Boot 3 (`src=f7404550`) paid queue-0 program.
Boot 2 (`src=ee650e06`) paid link classify. Boot 1 (`src=58b7448c`)
paid PHY identify. The earlier multi-boot capture `20260814-140651.log`
paid FDT + `rev=6.0`. The product capture `20260814-113438.log`
(`src=dcc997cc`) paid ADR-0102/0103.
