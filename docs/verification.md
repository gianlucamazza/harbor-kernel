...
12:54 is `src=f0bae772`; 12:31 is `src=df9a0d80`);
`make hw-check` judges the 12:54 PowerOn. That boot (`src=f0bae772`,
PowerOn, RBUF 64B+align) prints
...
the 124-byte TSB+probe after `RBUF_CTRL` 64B+align. The TSV window is
...
`RBUF_CTRL` 64B+align is not the missing send. None of those is a NIC.
