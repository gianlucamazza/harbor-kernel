...
The current capture is `.serial-log/20260814-174803.log` (one product boot);
`make hw-check` judges the last. Boot (`src=58b7448c`) prints
`genet: binding ok base=0xfd580000 len=0x10000 phy=rgmii-rxid (fdt, not probed)`,
`genet: rev=6.0 patch=0x0 (mmio, not a nic)`,
`genet: phy=0x600d84a2 (id, not a nic)`, `loader: store n=5 image`,
...
The previous multi-boot capture `20260814-140651.log` paid FDT + `rev=6.0`
(`src=61fe6774`) before this image selected `identify_phy`. The earlier
