...
12:31 is `src=df9a0d80`; 12:15 is `src=aa416a90`);
`make hw-check` judges the 12:31 PowerOn. That boot (`src=df9a0d80`,
PowerOn, RBUF TBUF size) prints
...
the 124-byte TSB+probe after `RBUF_TBUF_SIZE_CTRL=1`. The TSV window
is all zero. RX is timeout. The Apple NIC pcap across that boot has no
...
`RBUF_TBUF_SIZE_CTRL=1` is not the missing send. None of those is a NIC.
