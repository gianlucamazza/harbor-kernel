...
12:02 is `src=1349668e`; 11:52 is `src=89ced3d0`);
`make hw-check` judges the 12:02 PowerOn. That boot (`src=1349668e`,
PowerOn, TSB 64-byte) prints
...
the 124-byte TSB+probe. The TSV window is all zero. RX is timeout.
The Apple NIC pcap across that boot has no `0x88b5` and no source
`02:00:00:00:00:01` (only the laptop's DHCP). Serial complete is DMA
CONS, not a UniMAC send and not a wire frame. TSB-on is not the
missing send. None of those is a NIC.
