...
11:52 is `src=89ced3d0`; 11:17 is `src=b853980b`);
`make hw-check` judges the 11:52 PowerOn. That boot (`src=89ced3d0`,
PowerOn, default TX ring 0) prints
...
The first BMSR snapshot is down. TX printed complete (CONS posted) on
ring 0 the same way ring 16 did. The TSV window is all zero. RX is
timeout (no host frame). The Apple NIC pcap across that boot has no
...
The default TX ring is not the missing send. None of those is a NIC.
