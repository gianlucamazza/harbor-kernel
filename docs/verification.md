...
The current capture is `.serial-log/20260814-174803.log` (five product boots);
`make hw-check` judges the last. Boot 5 (`src=34b3d132`, PowerOn) prints
...
`genet: tx unavailable (link down)` after `frames:` (invariants
...
Cable-down is `link=down`, not a failure. The TX line is a refuse before
the doorbell, not a completed frame. A later image may print one
`RxReport` after Enabled; that line is unpaid.

Boot 4 (`src=0b1dc0b9`) paid queue-0 enable. Boot 3 (`src=f7404550`)
paid queue-0 program. Boot 2 (`src=ee650e06`) paid link classify.
Boot 1 (`src=58b7448c`) paid PHY identify. The earlier multi-boot
