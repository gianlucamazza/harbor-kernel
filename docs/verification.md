...
| Lints (`make clippy-kernel`, `make clippy-host`) | `--target aarch64-unknown-none-softfloat` over the kernel, then `-p kernel-core --all-targets` over the host crate and its tests| Everything clippy's lint set covers, over both halves of the tree — and it is the **largest** layer in `make check` by lines examined| Everything clippy does not lint, which is most of what the rows below are for. Its own blind spot until 2026-08-18 was structural rather than technical: these two passes lived in `check:`'s **recipe body**, not among its prerequisites, so make never reached them while any prerequisite was red. Six errors in `genet.rs`'s tests survived weeks of CI that way, and `layers-table` — which read the prerequisite line only — could not see them either. It reads the body now, and the passes are prerequisites, ordered before the gate that is expected to be red |
...
| Mutation scope (`make mutation-scope`)           | `docs/mutation-scope.toml` against the `pub mod` set of `crates/kernel-core/src/lib.rs`                                          | That every kernel-core module has a **recorded** mutation decision — `in_scope`, `queued`, or `exempt` with a reason. Forgetting a module becomes impossible; the run's `--file` list is derived from the same file, so there is no second copy (ADR-0049 trigger, ADR-0096)                                                                                                                                  | Whether the decision is a _good_ one. A module parked in `queued` is untested and honest about it; nothing here shortens that queue. It also says nothing about `src/`, which no mutation covers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Layers table (`make layers-table`)               | This section against the Makefile's `check:` prerequisites                                                                       | That every gate `make check` runs has a row here, and that every row states what it cannot see. `CONTRIBUTING.md` used to record that this table 'is checked by nobody'; when the gate was written it was already missing `panic-check` and `vocabulary-sync`                                                                                                                                                 | Whether a row is _true_. It compares membership and non-emptiness, never the content of a 'Covers' or 'Blind to' cell — a wrong blind spot reads exactly like a right one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| QEMU virtio transport (`make qemu-virtio-check`) | A `virt` boot with a deterministic peer, plus a second with no device| The accepted P3 path end to end (ADR-0104): modern feature negotiation, split-ring submission and completion, peer RX payload delivery, directional EL0 caps, reset lifecycle, and the absent-device refusal. **It was outside `make check` and outside CI until 2026-08-17** — the gate for the whole accepted network implementation was run only when someone remembered to, which is the thing ADR-0096 exists to end| Anything about GENET: `virt` has no BCM2711. The Pi 4 backend's only evidence is on silicon (ADR-0112 §5), and a green run here is not coverage of it |
| Hardware evidence (`make hw-evidence`)           | `docs/` citations against `docs/evidence/`, and each record against its capture where the capture is present                     | That every `YYYYMMDD-HHMMSS\*.log                                                                                                                                                                                                                                                                                                                                                                             | .pcap`a document cites exists as a tracked record, that no record sits there uncited, that each states the capture and sha256 it came from, and that re-deriving it reproduces it byte for byte (ADR-0109). Before this, 38 cited files lived only in the ignored`.serial-log/`and`make hw-check` had no input on a clone                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Whether the record is **true**, on a machine without the capture — there it checks a well-formed claim, not a verified one, and says how many of each. It is also blind by construction to anything inside an elided heartbeat run: a `frames_free` that sags mid-run and recovers survives only in the capture |
| Model consumed (`make model-consumed`)          | Reachability from `src/` into `crates/kernel-core/src/genet.rs`, against each pub item's doc comment| That a pure model is either reachable from the code that ships or carries a `Design-ahead (<slice>)` line naming what will consume it (ADR-0110). Written after `RingCursor` was found wrapping at 256 on a ring of 128 — a second copy of an advance `RingState` already owned, and the wrong copy had the green test| Whether the model is **correct**, and whether the named slice is real: a `Design-ahead` marker naming work nobody will do reads exactly like one naming next week's. It is also blind to an item that is reachable but whose result the driver ignores, and it covers `genet.rs` alone — widening it to all of `kernel-core` would refuse most of the crate on day one |
...
| Shipped store audit (`make hw-store-audit`)      | `inspect-store.py --elf … --image …` against a hardware transcript                                                               | That the composition **in the image that booted** is the one the board reported loading — count, agent names, and the `home_cpu` each was given. Two independent accounts of one composition: a host tool reading bytes off the artifact, and a kernel saying what it did with them. Refuses an ELF/image pair from different builds, and a transcript whose `build: … src=` is not this tree (P6 audit half) | Whether an agent did anything useful — `loader: … ran sends=…` belongs to the product oracle. And it audits the image **beside the tree**, not the bytes physically on the card: a card written by hand after the build would pass                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Lab x86 boot (`make x86-boot-check`)             | Boots `harbor-x86.elf` under `qemu-system-x86_64` q35, asserts L0 oracle                                                         | PVH entry → long mode, COM1 16550 TX, CPUID identity line ([ADR-0071](adr/0071-h3-l0-x86-qemu-first-slice.md))                                                                                                                                                                                                                                                                                                | Everything past L0 (timer, sched, SMP, bare-metal laptop). Until 2026-08-18 it was also in **neither** `make check` nor CI, so `done (QEMU-x86)` was a status claim nothing executed — the same shape as `qemu-virtio-check` the day before. It is a `check:` prerequisite now, and its ADR-0087 guard consults the measured share only on a **failure**: this guest boots in milliseconds and then `hlt`-loops, so a pre-assertion check called a perfectly good run INDETERMINATE at five hundredths of a core. A run that ended on its own is never INDETERMINATE either, or a corrupt image would report unmeasured forever instead of red                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| ADR-0105 + ADR-0106 | Pi 4 GENET v5 backend boundary and design | Host: `kernel_core::genet_fdt` and `kernel_core::genet` model the binding, the rings, the descriptors, MDIO, the PHY, the RGMII and UniMAC words, the HFB arithmetic and the boot sequence; `genet_emulation` is a deterministic non-hardware run. Silicon: thirty-one stamps between 2026-08-14 and 2026-08-17, from `binding ok` to a frame on the wire, are recorded boot by boot in [§Hardware evidence: Pi 4 GENET v5 bring-up](#hardware-evidence-pi-4-genet-v5-bring-up-2026-08-14--2026-08-17) — one row each, because a single table cell held all of them and nobody could read it (2026-08-17 review, F-10). The six items of the ADR-0105 evidence gate all exist in capture `20260817-105728.log`, and both ADRs were **accepted on that evidence 2026-08-17**. What is paid is the backend; `raspi4b` still prints `authority: network vocabulary VACANT` and P3 stays `done (QEMU)` until the backend is published to the network service. |
| ------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
method [ADR-0107](adr/0107-genet-sequence-first-bring-up.md), link
[ADR-0108](adr/0108-boot-path-link-acquisition.md).

This section exists because its contents used to be one 20 800-character table
cell. Every one of the thirty-one boots below was recorded honestly and
nothing was lost by writing them down; what was lost was the ability to read,
diff or review them. The 2026-08-17 excellence review filed that as F-10, and
this is the remediation: one row per boot, and the argument in prose beside
it rather than inside a cell.

### What the host model covers

`kernel_core::genet_fdt` validates the distributed Pi 4 DTB binding: ordered
MMIO translation, interrupt-parent, PHY, and every DMA aperture.

`kernel_core::genet` covers descriptor status and ownership, bounded rings,
reset generations, the queue-0 `RingProgram` (v5 word-unit start/end after the
descriptor RAM — a packet buffer is never a ring base), clause-22 `MdioTxn`
with absent-PHY-ID refusal, `PhyLink` BMCR/BMSR classification that requires
the binding's `rgmii-rxid`, the RGMII OOB words (`SYS_PORT_CTRL=EXT_GPHY`,
`RGMII_MODE_EN`, `OOB_DISABLE`/`ID_MODE_DIS` clear, `RGMII_LINK` only on Up),
UniMAC `UMAC_MAX_FRAME_LEN` and the station `UMAC_MAC0`/`UMAC_MAC1`, the
descriptor ring (Linux `DESC_INDEX` = 16) and the v5 default TX ring 0
(`TxRingSet` mask `0x1f` on TDMA `RING_CFG` and `RING_BUF_EN`; priority rings
1–4 at BD 128/160/192/224 × 32; RDMA `CTRL` stays the doorbell bit), TDMA
`ARB_CTRL=DMA_ARBITER_WRR` and `DMA_PRIORITY_0/1/2` (`WrrPriority::V5` =
`0x00421081`/`0`/`0`), `RBUF_CHK_CTRL` = `0x31`, the UniMAC datapath bits
(`TX_EN`/`RX_EN`/`PAD_EN`/`CRC_FWD`/`NO_LEN_CHK`) and taking `CMD_SW_RESET`
back off, the TSV window (`0x49c` packed trap, Linux `0x4a8` `tx_pkts`, `0x4ec`
`pok`), TBUF 64-byte TSB, `RBUF_TBUF_SIZE_CTRL=1`, `RBUF_CTRL`
`RBUF_ALIGN_2B|RBUF_64B_EN`, the SYS reset latch (`SYS_RBUF_FLUSH_CTRL`, which
is **not** `RBUF_CTRL`), the HFB packed-length and enable-word arithmetic
(Linux indexes filter lengths backwards from filter 47 and puts filters 0–31
in the _second_ enable word), ring-slot sizing against `RX_BUF_LENGTH`, RDMA's
`XON_XOFF_THRESH`, and `GenetBoot` as the unpublished sequence.

`invalidate_dcache_poc` is an AArch64 PoC operation, not a `qemu-virt`
feature. The `genet_emulation` tests are deterministic non-hardware runs:
a programmed queue 0 plus a PHY identify → reset-cleared → link-up path, and
an absent-ID refusal.

### What QEMU can and cannot say

QEMU has no successful GENET probe, so none of the `genet:` datapath lines are
asserted by an emulated oracle. What QEMU does hold to is the refusal side:
`make boot-check` requires `genet: unavailable (Missing)` when the Pi 4
fixture has its node deleted, and `genet: unavailable (no dtb)` on the
DTB-less boot. `make product-boot-check` requires the report line. Everything
below is silicon.

### The stamps

Every row is a PowerOn boot of the product image unless noted, and
`make hw-check` was clean on the capture at the time it was taken.

**Where the evidence lives** ([ADR-0109](adr/0109-hardware-evidence-is-tracked.md)).
The raw captures stay in `.serial-log/`, which is ignored: a capture is a
working artifact, and 99.95% of a long one is the idle heartbeat. What is
tracked is `docs/evidence/` — one derived record per cited capture, every
non-heartbeat line verbatim, each run of heartbeat collapsed to first + count +
last, with the source capture's sha256 in the header. So a filename cited in
this file resolves on a clone, and `make hw-check` runs against it:

```
make hw-check TRANSCRIPT=docs/evidence/20260817-105728.log
```

The oracle's verdict is the same on both — checked across all 34 captures cited
when the directory was created. `make hw-evidence` refuses a citation with no
record, a record no document cites, and a record that stops matching the
capture it names.

| When        | `src=`     | The slice                        | What the board said                                                                  | Reading                                                                                                 |
| ----------- | ---------- | -------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| 08-14       | `1aa3e894` | FDT report only                  | `binding ok base=0xfd580000 len=0x10000 phy=rgmii-rxid`, `network vocabulary VACANT` | The window is mapped on every boot; not a probe                                                         |
| 08-14       | `61fe6774` | first MMIO probe                 | `rev=6.0 patch=0x0`                                                                  | The block answers; not a NIC                                                                            |
| 08-14       | `58b7448c` | PHY identify                     | `phy=0x600d84a2`                                                                     | Clause-22 reaches the PHY                                                                               |
| 08-14       | `ee650e06` | BMSR classify                    | `link=down`                                                                          | A sample, not a claim                                                                                   |
| 08-14       | `f7404550` | queue-0 program                  | `queue0 programmed` after `frames:`                                                  | Rings written, DMA still off                                                                            |
| 08-14       | `0b1dc0b9` | queue-0 enable                   | `queue0 enabled`                                                                     | DMA on                                                                                                  |
| 08-14       | `34b3d132` | first TX attempt                 | `tx unavailable (link down)`                                                         | A refusal before the doorbell                                                                           |
| 08-14       | `9cd383eb` | first RX attempt                 | `rx unavailable (link down)`                                                         | A refusal before `RX_EN`                                                                                |
| 08-14       | `30603cba` | recover                          | `reset recovered (idle)`                                                             | Back to Idle                                                                                            |
| 08-14       | `30603cba` | (2nd capture)                    | `tx/rx unavailable (timeout)`; laptop RX=0                                           | Neither completed                                                                                       |
| 08-15       | `3ee58791` | UniMAC speed                     | still `timeout`; RX=0                                                                | Not a `CMD_SPEED_*` claim                                                                               |
| 08-15       | `d53194dc` | RGMII OOB                        | `rgmii oob (ext-gphy)`, still `timeout`                                              | Not the missing send                                                                                    |
| 08-15       | `6addf8f4` | UniMAC frame/station             | `umac init (frame)`, still `timeout`                                                 | Not the missing send                                                                                    |
| 08-15       | `8b896ceb` | ring 16                          | `desc ring programmed/enabled`, still `timeout`                                      | Not the missing send                                                                                    |
| 08-15       | `fa00d083` | OWN writeback                    | `tx unavailable (still owned)`                                                       | CONS posted; silicon does not write OWN back on TX                                                      |
| 08-15       | `19b6438d` | CONS retire                      | `tx complete len=60`; pcap empty                                                     | CONS is not a send                                                                                      |
| 08-15       | `a7891eb1` | TSV read                         | `tx complete len=60`, `umac tx pkts=0`                                               | The counter is the honest oracle, and it is zero                                                        |
| 08-16       | `b853980b` | TBUF raw + max-pkt               | `tsv packed=0 linux=0 pok=0`                                                         | The 0xC MIB hole is real; not the missing send                                                          |
| 08-16       | `89ced3d0` | ring 0 vs ring 16                | same zeros                                                                           | Ring 0 CONS-posts like ring 16                                                                          |
| 08-16       | `1349668e` | TSB on                           | `tx complete len=124`, same zeros                                                    | TSB is not the missing send                                                                             |
| 08-16       | `aa416a90` | no pre-doorbell flush            | same zeros                                                                           | That flush is not the missing send                                                                      |
| 08-16       | `df9a0d80` | `RBUF_TBUF_SIZE_CTRL=1`          | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `f0bae772` | `RBUF_CTRL` 64B+align            | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `5bf281a0` | TDMA WRR arbiter                 | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `807c9f52` | `RING_CFG=0x1f`                  | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `d1cfbca5` | honest CONS name                 | `tx cons len=124`, same zeros                                                        | A rename, not a send                                                                                    |
| 08-16       | `656be102` | `RING_BUF_EN=0x1f`               | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `7d1631b4` | TX rings 1–4                     | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `414f4098` | WRR priority words               | same zeros                                                                           | Not the missing send                                                                                    |
| 08-16       | `3f2d01b8` | `init_phy` on the boot path      | `phy init`, `tx unavailable (link down)`                                             | **A regression**: the BMCR reset dropped the link it then measured                                      |
| 08-17 11:00 | `8981a0dc` | ADR-0108: no PHY reset in boot   | no `phy init`; `tx cons len=124`, `rx unavailable (timeout)`                         | Regression undone; probe sample down, submit sample up                                                  |
| 08-17 11:00 | `8981a0dc` | ADR-0107 first sequence claim    | `hfb cleared`, still `tsv 0/0/0`; pcap empty, host NIC RX=0                          | Order, settles, HFB, descriptor words and ring geometry are **together** not the missing send           |
| 08-17 11:10 | `4c822c78` | SYS latch as the reset target    | `probe unavailable (Timeout)`                                                        | **A failure that is evidence**: the self-clear poll had only ever passed because the write never landed |
| 08-17 11:15 | `9b074c54` | no poll + state dump             | `state sysrbuf=0x0 … cmd=0x1002067 …`                                                | Everything reads back correct except `CMD_SW_RESET`, still set 300 ms on                                |
| 08-17 11:21 | `4616443a` | `UMAC_CMD = 0` after the settle  | `rx complete len=160`; **first `0x88b5` on the wire**; `cmd=0x1000067`               | The reset bit was the blocker                                                                           |
| 08-17 11:27 | `5987d6c`  | bounded TSV settle               | `tsv packed=0 linux=1 pok=1`; second `0x88b5`                                        | UniMAC counts the packet; one frame per boot closes attribution                                         |
| 08-17 11:34 | `0a937a23` | wall-clock RX window             | `tx cons`+`tsv 0/1/1`, `rx complete len=168`, `reset recovered`; third `0x88b5`      | Five gate items in one boot                                                                             |
| 08-17 11:55 | `a4b40bb3` | boot description without the NIC | `unavailable (Missing)`, `probe unavailable (no binding)`                            | The sixth: a refusal, not an invented binding                                                           |

### What the sequence of stamps says, that no single one does

Twenty-five of those rows are one register each, and none of them moved the
outcome. That is not twenty-five wasted boots: it is what falsified the
hypothesis they were all testing. The defect was never a missing register — it
was **where** the registers were written, and then that UniMAC was in software
reset the whole time. A one-variable slice cannot find either, because every
slice put its variable at the same wrong point.

Four things only the state dump could settle, and it settled them in one boot:
the SYS latch was clear, the RGMII block was configured, both DMA engines were
running, and the retired TX BD held exactly what the driver wrote. One word out
of eleven was wrong, and it was the one that disables the MAC.

### The gate

[ADR-0105](adr/0105-pi4-nic-backend-boundary.md) asks for a serial capture on
a real Pi 4 proving probe, link state, one bounded TX, one bounded RX,
reset/recovery, and absent-device refusal. All six exist in
`20260817-105728.log`: the first five at 11:34 (boot 7, `src=0a937a23`) and the
sixth at 11:55 (boot 8, `src=a4b40bb3`). Necessarily two boots — a boot cannot
describe the device as both present and absent — and the two images differ by
commits that touch no kernel code (`git diff --stat 0a937a23 a4b40bb3 -- src
crates` is empty).

An earlier absent-NIC boot at 11:49 shows the same two lines but lost its
`Harbor: hello` when the serial adapter resynchronised across the power cycle,
so `hw-transcript-check` could not isolate it. A boot nobody can isolate is a
reading, not evidence; it was captured again rather than argued about, which is
what the script's own note asks for.

The absent-device boot description is built by
`scripts/host/absent-nic-dtb.sh` from the tracked fixture — same firmware pin
as `third_party/blobs`, hash re-checked immediately before use — with
`/scb/ethernet`, its aliases and its `__symbols__` labels deleted, and it
asserts its own result. `make deploy-absent-nic` deploys it; a plain `make
deploy` removes it again, so no later boot can inherit a description that would
look exactly like a NIC regression.

**ADR-0105 and ADR-0106 were accepted on this evidence on 2026-08-17** by the
project owner. `raspi4b` has a NIC backend and its hardware evidence.

It does not have a _published_ one. The backend still sits below the EL1
network service, the product still prints
`authority: network vocabulary VACANT`, and binding it to the network
vocabulary is the BSP composition step ADR-0105 named from the start. P3
therefore stays `done (QEMU)`: what acceptance paid is the backend and its
evidence, not the composition.
