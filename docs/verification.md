...
12:15 is `src=aa416a90`; 12:02 is `src=1349668e`);
`make hw-check` judges the 12:15 PowerOn. That boot (`src=aa416a90`,
PowerOn, no pre-doorbell flush) prints
...
the 124-byte TSB+probe without a pre-doorbell `UMAC_TX_FLUSH`. The TSV
...
frame. Skipping the pre-doorbell flush is not the missing send. None
of those is a NIC.
