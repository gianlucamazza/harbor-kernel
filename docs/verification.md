...
13:45 is `src=d1cfbca5`; 13:27 is `src=807c9f52`);
`make hw-check` judges the 13:45 PowerOn. That boot (`src=d1cfbca5`,
PowerOn, `tx cons`) prints
...
124-byte TSB+probe after TDMA `RING_CFG=0x1f`. The TSV window is all
zero. RX is timeout. The Apple NIC pcap across that boot has no
`0x88b5` and no source `02:00:00:00:00:01` (only the laptop's DHCP).
`tx cons` is DMA CONS, not a UniMAC send and not a wire frame. None
of those is a NIC.
