...
| ADR-0077            | SMP shared-state discipline                   | Heap + sched + park under `IrqSpinLock`; single `arch::smp` resched; per-CPU mirrors. **F-R1-P1 complete (QEMU):** also IPC, frames, ASID, naming, storage, taskcap, durable, console TX/RX model, IRQ wait/caps, MMU map/unmap. First silicon exercise with ADR-0076 stamp (`20260810-130305.log`); full table set exercised on every dual-current `boot-check` after 2026-08-10. Residual: lock refinement if measured; agent+TLB steal policy                                                                 |
