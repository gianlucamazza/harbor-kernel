...
The current capture is `.serial-log/20260814-232303.log` (six product
boots with the laptop as link partner); `make hw-check` judges boot 6.
Boot 6 (`src=8b896ceb`, PowerOn, descriptor ring 16) prints
...
The first BMSR snapshot is down; the TX/RX lines are timeout, not
`unknown speed`, not `link down`, and not `complete`. The laptop Apple
NIC (`enp0s20f0u5u2`, MAC `08:6d:41:e5:db:60`) received 0 frames.
Descriptor ring 16 ran; it did not put a frame on the wire. None of
those is a completed frame or a NIC.
