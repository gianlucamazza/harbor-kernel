...
| Mutation scope (`make mutation-scope`)           | `docs/mutation-scope.toml` against the `pub mod` set of `crates/kernel-core/src/lib.rs`                                          | That every kernel-core module has a **recorded** mutation decision — `in_scope`, `queued`, or `exempt` with a reason. Forgetting a module becomes impossible; the run's `--file` list is derived from the same file, so there is no second copy (ADR-0049 trigger, ADR-0096) | Whether the decision is a *good* one. A module parked in `queued` is untested and honest about it; nothing here shortens that queue. It also says nothing about `src/`, which no mutation covers |
| Layers table (`make layers-table`)               | This section against the Makefile's `check:` prerequisites| That every gate `make check` runs has a row here, and that every row states what it cannot see. `CONTRIBUTING.md` used to record that this table 'is checked by nobody'; when the gate was written it was already missing `panic-check` and `vocabulary-sync` | Whether a row is *true*. It compares membership and non-emptiness, never the content of a 'Covers' or 'Blind to' cell — a wrong blind spot reads exactly like a right one |
| Composition vocabulary (`make vocabulary-sync`)  | `src/bootstrap/authority.rs` against `scripts/agent/pack-store.py`| That the names and indices of the composition vocabulary agree in both directions (ADR-0099). A store entry grants authority by writing an **index**; a disagreement would grant whatever the kernel declared at that index, with the loader reporting `refusals=0` | Whether an index is actually **minted** at boot. That is a runtime fact, and `authority: N <name> VACANT` on the wire is where it shows |
...
| ADR-0105 + ADR-0106 | Pi 4 GENET v5 backend boundary and design | Host: `kernel_core::genet_fdt` validates the distributed Pi 4 DTB binding, ordered MMIO translation, interrupt-parent, PHY and all DMA apertures; `kernel_core::genet` covers descriptor status, ownership, bounded rings, reset generations, queue-0 `RingProgram` (v5 word-unit start/end after descriptor RAM; a packet buffer is never a ring base), clause-22 `MdioTxn` plus absent PHY-ID refusal, `PhyLink` BMCR/BMSR classification that requires the binding's `rgmii-rxid`, RGMII OOB words (`SYS_PORT_CTRL=EXT_GPHY`, `RGMII_MODE_EN`, `OOB_DISABLE`/`ID_MODE_DIS` clear, `RGMII_LINK` only on Up), UniMAC `UMAC_MAX_FRAME_LEN` plus station `UMAC_MAC0`/`UMAC_MAC1`, the descriptor-based ring (Linux `DESC_INDEX` = 16) Linux v5 default TX ring 0 (`TxRingSet` mask `V5_TX_RING_CFG=0x1f` on TDMA `RING_CFG` and TDMA `RING_BUF_EN`; priority rings 1–4 at BD 128/160/192/224 × 32; RDMA `CTRL` stays the doorbell bit), TDMA `ARB_CTRL=DMA_ARBITER_WRR`, TDMA `DMA_PRIORITY_0/1/2` (`WrrPriority::V5` = `0x00421081`/`0`/`0`), a bounded BMCR reset after identify (`PhyInitReport`), `RBUF_CHK_CTRL` (`rbuf_chk_ctrl(_, true)` = `0x31`), CONS retire as `TxReport::ConsPosted` (not a UniMAC send), `GenetBoot` as the unpublished sequence, a TX/RX poll that distinguishes MDIO timeout, CONS-not-posted, and still-owned, TX complete on CONS (OWN writeback is not how GENET TX finishes), TX descriptors with `APPEND_CRC` and v5 `QTAG`, UniMAC datapath bits (`TX_EN`/`RX_EN`/`PAD_EN`/`CRC_FWD`/`NO_LEN_CHK`), a `TX_FLUSH` pulse at UniMAC init (not on each doorbell), the UniMAC TSV window (`0x49c` packed trap, Linux `0x4a8` `tx_pkts`, `0x4ec` `pok`), TBUF 64-byte TSB (`TBUF_64B_EN`, `TX_DMA_BYTES=124`), `RBUF_TBUF_SIZE_CTRL=1`, `RBUF_CTRL` `RBUF_ALIGN_2B|RBUF_64B_EN`, leftover `SYS_TBUF_FLUSH` release, `UMAC_MIB_CTRL` pulse-and-clear, and ring-16 TDMA `FLOW_PERIOD` `MAX_FRAME << 16`. The `genet_emulation` tests are deterministic non-hardware runs: programmed queue 0 plus a PHY identify → reset-cleared → link-up path and an absent-ID refusal. AArch64 `src/drivers/genet.rs` can write that program, run `init_phy`, enable queue 0 only after `DmaPhase::Programmed`, and submit one bounded TX and one bounded RX after Enabled; `TxReport::refuse` / `RxReport::refuse` block the doorbell or RX arm when BMSR is down. After those reports `recover` returns the controller to Idle (`ResetReport`). `invalidate_dcache_poc` is an AArch64 PoC operation, not a `qemu-virt` feature. `board-rpi4` selects those unpublished slices and still leaves the network vocabulary vacant. A 2026-08-14 Pi 4B oracle boot stamp now proves the board baseline, and the product boot `20260814-113438.log` (`src=dcc997cc`) likewise leaves `authority: network vocabulary VACANT`. The product now prints a `genet:` FDT report (`binding ok` or `unavailable`) without probing MMIO or binding the network service; `make product-boot-check` and `make boot-check` require the line. QEMU product-boot is DTB-less (`unavailable (no dtb)`); QEMU with the Pi 4 fixture deletes the GENET node (`unavailable (Missing)`). **Pi stamp 2026-08-14, transcript `20260814-140651.log`** — product image on silicon (`src=1aa3e894`): `genet: binding ok base=0xfd580000 len=0x10000 phy=rgmii-rxid (fdt, not probed)` and `authority: network vocabulary VACANT`. `make hw-check` clean. The compiled 64 KiB window is mapped on every `board-rpi4` boot. A matching FDT binding runs `Genet::probe`; otherwise the product prints `genet: probe unavailable (no binding)` (QEMU) or `probe skipped`. **Pi stamp 2026-08-14, same transcript, boot 4 (`src=61fe6774`)**: `genet: rev=6.0 patch=0x0 (mmio, not a nic)`. After a successful revision the product prints one `PhyIdentify` line. **Pi stamp 2026-08-14, transcript `20260814-174803.log` (`src=58b7448c`)**: `genet: phy=0x600d84a2 (id, not a nic)`. After a successful identify the product prints one `LinkReport` line (`genet: link=up|down (bmsr, not a nic)`). **Pi stamp 2026-08-14, same transcript, boot 2 (`src=ee650e06`)**: `genet: link=down (bmsr, not a nic)`. **Pi stamp 2026-08-14, same transcript, boot 3 (`src=f7404550`)**: `genet: queue0 programmed (rings, not a nic)` after `frames:` (`frames_free=510`). **Pi stamp 2026-08-14, same transcript, boot 4 (`src=0b1dc0b9`)**: `genet: queue0 enabled (dma, not a nic)` after `queue0 programmed`. After Enabled the product prints one `TxReport` line (`tx unavailable (link down)` or `tx complete len=…` or timeout). **Pi stamp 2026-08-14, same transcript, boot 5 (`src=34b3d132`, PowerOn)**: `genet: tx unavailable (link down)` after `queue0 enabled`. That is a refuse before the doorbell, not a completed TX. **Pi stamp 2026-08-14, same transcript, boot 6 (`src=9cd383eb`, PowerOn)**: `genet: rx unavailable (link down)` after the TX refuse. That is a refuse before UniMAC `RX_EN`, not a completed RX. **Pi stamp 2026-08-14, same transcript, boot 7 (`src=30603cba`, PowerOn)**: `genet: reset recovered (idle, not a nic)` after the TX/RX refuses. That is a return to Idle, not a NIC. **Pi stamp 2026-08-14, transcript `20260814-232303.log` (`src=30603cba`, PowerOn)**: first BMSR `link=down`, then `tx unavailable (timeout)` / `rx unavailable (timeout)` after enable; laptop RX=0. That is not a completed TX or RX. **Pi stamp 2026-08-15, same transcript, boot 3 (`src=3ee58791`, PowerOn)**: UniMAC speed slice still printed `tx/rx unavailable (timeout)` (not `unknown speed`, not complete); laptop Apple RX=0. That is not a written-`CMD_SPEED_*` claim. **Pi stamp 2026-08-15, same transcript, boot 4 (`src=d53194dc`, PowerOn)**: `genet: rgmii oob (ext-gphy, not a nic)` then still `tx/rx unavailable (timeout)`; laptop RX=0. **Pi stamp 2026-08-15, same transcript, boot 5 (`src=6addf8f4`, PowerOn)**: `genet: umac init (frame, not a nic)` then still `tx/rx unavailable (timeout)`; laptop RX=0. **Pi stamp 2026-08-15, same transcript, boot 6 (`src=8b896ceb`, PowerOn)**: `desc ring programmed (16)` / `enabled` then still `tx/rx unavailable (timeout)`; laptop RX=0. **Pi stamp 2026-08-15, same transcript, boot 7 (`src=fa00d083`, PowerOn)**: `tx unavailable (still owned)`, `rx unavailable (timeout)`; laptop RX=0. CONS posted; OWN was not written back. **Pi stamp 2026-08-15, same transcript, boot 8 (`src=19b6438d`, PowerOn)**: `tx complete len=60 (one frame, not a nic)`, `rx unavailable (timeout)`; laptop RX=0. CONS posted on serial; no `0x88b5` on the host counters. **Pi stamp 2026-08-15, transcript `20260815-092435.log` + pcap `20260815-103422-apple-nic.pcap` (`src=19b6438d`, PowerOn)**: serial again `tx complete len=60`; the pcap has no `0x88b5` and no `02:00:00:00:00:01`. **Pi stamp 2026-08-15, same transcript, boot 2 (`src=a7891eb1`, PowerOn)**: `tx complete len=60` then `umac tx pkts=0`; pcap still has no `0x88b5`. CONS posted; UniMAC TSV is zero. **Pi stamp 2026-08-16, transcript `20260816-052739.log` (`src=a7891eb1`, PowerOn)**: same two lines after a product reflash; live pcap still has no `0x88b5`. That is not a wire frame. **Pi stamp 2026-08-16, same transcript, 11:17 (`src=b853980b`, PowerOn)**: `tbuf raw`, `tx complete len=60`, `umac tsv packed=0 linux=0 pok=0`; live pcap still has no `0x88b5`. The 0xC hole is real; TBUF raw + ring-16 max-pkt did not produce a UniMAC send. **Pi stamp 2026-08-16, same transcript, 11:52 (`src=89ced3d0`, PowerOn)**: `queue0 programmed` / `enabled`, same TSV zeros, pcap still empty. Ring 0 CONS-posts like ring 16; it is not the missing send. **Pi stamp 2026-08-16, same transcript, 12:02 (`src=1349668e`, PowerOn)**: `tbuf tsb`, `tx complete len=124`, TSV still zeros, pcap still empty. TSB-on is not the missing send. **Pi stamp 2026-08-16, same transcript, 12:15 (`src=aa416a90`, PowerOn)**: no pre-doorbell flush, same TSV zeros, pcap still empty. Skipping that flush is not the missing send. **Pi stamp 2026-08-16, same transcript, 12:31 (`src=df9a0d80`, PowerOn)**: `tbuf size`, same TSV zeros, pcap still empty. `RBUF_TBUF_SIZE_CTRL=1` is not the missing send. **Pi stamp 2026-08-16, same transcript, 12:54 (`src=f0bae772`, PowerOn)**: `rbuf 64b`, same TSV zeros, pcap still empty. `RBUF_CTRL` 64B+align is not the missing send. **Pi stamp 2026-08-16, same transcript, 13:07 (`src=5bf281a0`, PowerOn)**: `tdma arb`, same TSV zeros, pcap still empty. TDMA `ARB_CTRL=DMA_ARBITER_WRR` is not the missing send. **Pi stamp 2026-08-16, same transcript, 13:27 (`src=807c9f52`, PowerOn)**: `ring cfg`, same TSV zeros, pcap still empty. TDMA `RING_CFG=0x1f` is not the missing send. **Pi stamp 2026-08-16, same transcript, 13:45 (`src=d1cfbca5`, PowerOn)**: `tx cons len=124`, same TSV zeros, pcap still empty. The honest CONS name is on the wire; it is not a UniMAC send. **Pi stamp 2026-08-16, same transcript, 13:58 (`src=656be102`, PowerOn)**: `ring buf`, same TSV zeros, pcap still empty. TDMA `RING_BUF_EN=0x1f` is not the missing send. **Pi stamp 2026-08-16, same transcript, 14:17 (`src=7d1631b4`, PowerOn)**: `rings 1-4`, same TSV zeros, pcap still empty. Programming TDMA rings 1–4 is not the missing send. **Pi stamp 2026-08-16, same transcript, 14:28 (`src=414f4098`, PowerOn)**: `tdma prio`, same TSV zeros, pcap still empty. `DMA_PRIORITY_0/1/2` is not the missing send. **Pi stamp 2026-08-16, same transcript, 14:39 (`src=3f2d01b8`, PowerOn)**: `phy init`, `tx unavailable (link down)`, same TSV zeros, pcap still empty. BMCR reset dropped the submit link; it is not the missing send. **Pi stamp 2026-08-17, transcript `20260817-105728.log` (`src=8981a0dc`, PowerOn, boot 2 of 2)** ([ADR-0108](adr/0108-boot-path-link-acquisition.md)): the boot path no longer resets the PHY. `reset_phy` restarts 1000BASE-T autonegotiation, which takes seconds, so the BMSR read in the same phase could not observe a link — the 14:39 stamp is that outcome, not a discovery. Identify and classify stay; `Genet::reset_phy` stays on the driver for a slice that can wait. The boot prints `phy=0x600d84a2` and `link=down` with **no `phy init` line**, and the doorbell is reached again: `tx cons len=124` and `rx unavailable (timeout)` replace the `(link down)` pair. The probe sample is down and the submit sample is up — autonegotiation finishes between them, which is exactly what the same-phase read could not wait for. The 14:39 regression is undone. **Pi stamp 2026-08-17, same transcript — paid, negative** ([ADR-0107](adr/0107-genet-sequence-first-bring-up.md)): one coherent claim, not one register. UniMAC, TBUF, RBUF, `RBUF_CHK_CTRL`, the RGMII block and a full `bcmgenet_hfb_clear` now run **before** `DMA_EN`, which Linux writes last (`init_umac` then `bcmgenet_init_dma`); `UMAC_TX_FLUSH` and the `RBUF_CTRL` latch are pulsed in `init_dma`'s position with real 10 µs settles instead of a register readback; the reset path gains Linux's 10 µs and 2 µs waits; `HFB_CTRL`, both enable words, the eight RDMA index-to-ring words and all 48 filters are cleared and filter 0 is re-enabled with length 4 (Linux's default flow to ring 0), which Harbor had never done; the TX descriptor drops `DMA_OWN` and `DMA_WRAP`, which `bcmgenet_xmit` never sets, and the RX descriptor is written address-only, as `bcmgenet_rx_refill` does; TX ring 0 owns 128 BDs and RX ring 0 owns 256 with `RX_BUF_LENGTH` slots, so ring 0 no longer disagrees with the rings 1–4 it is placed against; and RDMA's per-ring `FLOW_PERIOD` offset is written as `XON_XOFF_THRESH` rather than zero. Host-tested: HFB packed-length and enable-word arithmetic (Linux indexes lengths backwards from filter 47 and puts filters 0–31 in the *second* enable word), filter-RAM bounds, ring-slot sizing, and the index-to-ring block. On silicon the boot prints `genet: hfb cleared (flow0, not a nic)`, `tx cons len=124`, `umac tsv packed=0 linux=0 pok=0` and `rx unavailable (timeout)`; the pcap `20260817-105728-apple-nic.pcap` holds only the host's own frames — no `0x88b5`, no `02:00:00:00:00:01` — and the host NIC's own counter read **RX 0 packets** across the boot, an oracle independent of the capture. The init order, the real settles, HFB, the descriptor words and ring 0's geometry are **together** not the missing UniMAC send. Note the printed order is `GenetBoot`'s field order, not the execution order: the reorder is in `boot_after_program`, not in the transcript. Per ADR-0107 §4 the next slice is a **read-only state dump**, not another register: the remaining class is clock/power to the RGMII block and firmware-left controller state. **Pi stamp 2026-08-17, same transcript, 11:10 (`src=4c822c78`, PowerOn): `genet: probe unavailable (Timeout)`** — a failed boot that is evidence. The slice targeted `SYS_RBUF_FLUSH_CTRL` (SYS `0x08`) instead of `RBUF_CTRL` (RBUF `0x300`): Linux reaches both through `bcmgenet_rbuf_ctrl_get/set`, whose name does not say `SYS` (`bcmgenet.c:127-140`), and Harbor had been zeroing the RBUF one — so the latch that holds UniMAC in reset had never been touched, and `bcmgenet_umac_reset` (`:3299-3311`, called at `:3368` before `init_umac`) had no analogue at all. Releasing the latch made the probe time out on a poll Harbor had invented: `reset()` waited for `CMD_SW_RESET` to self-clear, a contract Linux never claims — `reset_umac` asserts it, waits 2 µs and stops. **That poll had been passing because the write never landed**: with the MAC held by the SYS latch, `UMAC_CMD` read back as zero and the loop exited on its first iteration. The timeout is therefore evidence *for* the diagnosis, not against it. The poll is removed and the state dump reports the bit instead. **Pi stamp 2026-08-17, same transcript, 11:15 (`src=9b074c54`, PowerOn) — the state dump, and the first mechanism-level answer in twenty-eight boots.** `genet: state sysrbuf=0x0 port=0x3 oob=0xf00050 cmd=0x1002067 rbuf=0xc043 tbuf=0x1 tdma=0x3f/0x3ffc0 rdma=0x3/0x3fffc txbd=0x7c7fc0 (read, not a nic)`. Everything the sequence programs reads back correct: the SYS latch is clear (so *that* hypothesis is refuted), `SYS_PORT_CTRL` is `EXT_GPHY`, the RGMII block has `RGMII_MODE_EN` and `RGMII_LINK` set with `OOB_DISABLE`/`ID_MODE_DIS` clear, both DMA engines report enabled and not disabled, and the retired TX BD holds length 124 with `SOP`/`EOP`/`APPEND_CRC`/qtag and no `OWN` writeback. One word is wrong: `cmd=0x1002067` has **bit 13, `CMD_SW_RESET`, still set** — together with `TX_EN`, `RX_EN`, `PAD_EN`, `CRC_FWD` and `NO_LEN_CHK`, almost 300 ms after the reset. UniMAC was in software reset for the whole boot, with the datapath enables written over the top; that is precisely the state in which `umac_enable_set` refuses to write `UMAC_CMD` at all (`bcmgenet.c:2540-2545`), and precisely the shape of *TDMA retires the descriptor and UniMAC counts nothing*. `reset_umac` asserts that bit and stops, nothing in Linux clears it, and on BCM2711 it does not self-clear. Harbor now writes `UMAC_CMD = 0` after the settle and `umac_cmd_datapath` clears the bit defensively. Host-tested against the observed word. Silicon unpaid. **Pi stamp 2026-08-17, same transcript, 11:21 (`src=4616443a`, PowerOn) — the first positive GENET evidence in the project's history.** Serial: `genet: rx complete len=160 (one frame, not a nic)` — a real received frame, where every previous boot printed `timeout` or `link down`. Wire: the capture `20260817-105728-apple-nic.pcap` contains, for the first time, `02:00:00:00:00:01 > ff:ff:ff:ff:ff:ff, ethertype 0x88b5, length 60`, and the host NIC's own counter moved from RX 0 to **RX 1 packet / 46 bytes**. Exactly one such frame exists in the whole capture. The state dump now reads `cmd=0x1000067` — bit 13 clear — with everything else unchanged. Taking UniMAC out of software reset is what moved both. Two things are **not** claimed. The TSV still printed `packed=0 linux=0 pok=0`, because it is sampled microseconds after the doorbell and the frame reached the host later; that is a read-too-early, and the next slice gives it a bounded settle. And the pcap timestamp (`.585536`) precedes the serial `tx cons` line (`.774124`), which does **not** establish that the frame is this boot's TX: the serial log lags execution by tens of milliseconds at 115200 with a full boot's output queued (`queue0 programmed` to `hfb cleared` spans 45 ms of UART for work that takes microseconds). Attribution needs a second boot producing a second frame. **Pi stamp 2026-08-17, same transcript, 11:27 (`src=5987d6c`, PowerOn) — the TX is confirmed by the MAC itself and attributed to the boot that reports it.** `genet: tx cons len=124 (dma, not a nic)` then `genet: umac tsv packed=0 linux=1 pok=1 (mib, not a nic)`: with the bounded settle in place, UniMAC counts one transmitted packet and one good packet. The capture holds a **second** `0x88b5` frame from `02:00:00:00:00:01` at `11:27:05.493350`, and the host NIC counter reads RX 2 packets. One frame per boot closes the attribution question the 11:21 stamp had to leave open: the frame belongs to the boot that prints the counter. The dump is unchanged (`cmd=0x1000067`). RX this boot printed `timeout`, where 11:21 printed `rx complete len=160` on the same code — the difference is whether a broadcast landed inside a spin count, which is a fact about the laptop rather than about Harbor, so the RX bound is now a wall-clock window. **What the ADR-0105 gate still lacks**: one capture containing a bounded TX **and** a bounded RX in the *same* boot, and an absent-device refusal on the Pi. Probe, link state, TX, RX, and reset/recovery each exist on silicon; they do not yet exist together. ADR-0105/0106 stay proposed. **Pi stamp 2026-08-17, same transcript, 11:34 (`src=0a937a23`, PowerOn, boot 7 of 7) — probe, link, a bounded TX and a bounded RX and reset/recovery, in one boot.** `genet: rev=6.0 patch=0x0`, `genet: phy=0x600d84a2`, `genet: link=down (bmsr)` at probe, then `genet: tx cons len=124 (dma, not a nic)`, `genet: umac tsv packed=0 linux=1 pok=1 (mib, not a nic)`, `genet: rx complete len=168 (one frame, not a nic)`, `genet: state sysrbuf=0x0 … cmd=0x1000067 …`, `genet: reset recovered (idle, not a nic)`. `make hw-check` clean. The capture holds a **third** `0x88b5` frame from `02:00:00:00:00:01` — one per boot since UniMAC came out of software reset. The RX window is wall-clock and the link partner was sending a broadcast every 200 ms, so the RX is a property of the code rather than of the laptop's timing. **One item of the ADR-0105 gate is still absent: an absent-device refusal on the Pi.** Harbor produces that refusal on QEMU (`unavailable (Missing)`, `unavailable (no dtb)`), which the ADR labels non-hardware evidence; on real silicon the node is always present unless the boot description is changed deliberately. Until that is produced and the owner accepts it, ADR-0105/0106 stay proposed and `raspi4b` has no NIC backend. **Pi stamp 2026-08-17, same transcript, 11:55 (`src=a4b40bb3`, PowerOn, boot 8 of 8, `make hw-check` clean) — the absent-device refusal, on silicon.** The card carried a boot description built by `scripts/host/absent-nic-dtb.sh` from the tracked fixture with `/scb/ethernet`, its aliases and its `__symbols__` labels removed. The firmware patched that blob as it patches its own — `discover: model "Raspberry Pi 4 Model B Rev 1.5" rev=0xc03115 (fdt)`, memory and cpus unchanged — so the only difference in the boot is that GENET is not described. Harbor printed `genet: unavailable (Missing)` and `genet: probe unavailable (no binding)`: it refused rather than invent a binding for a device the description does not have. `make deploy-absent-nic` builds and deploys that pair; a plain `make deploy` removes the description again, so no later boot can inherit it. **With this the six items of the ADR-0105 evidence gate all exist in one capture** (`20260817-105728.log`): probe, link state, one bounded TX, one bounded RX and reset/recovery at 11:34 (`src=0a937a23`, boot 7), and the absent-device refusal at 11:55 (`src=a4b40bb3`, boot 8). An earlier absent-NIC boot at 11:49 shows the same two lines but lost its `Harbor: hello` when the adapter resynchronised across the power cycle, so `hw-check` could not isolate it; it was captured again rather than argued about, which is what the script's own note asks for. They are necessarily in two boots — a boot cannot describe the device as both present and absent — and the two images differ by commits that touch no kernel code (`git diff --stat 0a937a23 a4b40bb3 -- src crates` is empty). Accepting ADR-0105 and ADR-0106 on that evidence is the ADR owner's decision, not the driver's; until it is taken they stay `proposed` and `raspi4b` has no NIC backend. QEMU has no successful probe so it prints none of those lines. That is not a NIC claim. No GENET-capable QEMU device, bound MMIO backend, or Pi 4 NIC capture exists; GENET remains unclaimed. ADR-0105/0106 stay proposed. |
| ------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
The current capture is `.serial-log/20260816-052739.log` (boot at
14:39 is `src=3f2d01b8`; 14:28 is `src=414f4098`);
`make hw-check` judges the 14:39 PowerOn. That boot (`src=3f2d01b8`,
PowerOn, BMCR reset) prints
...
`genet: reset recovered (idle, not a nic)` after `frames:`,
`loader: store n=5 image`, and `invariants: … slots=4/9`.
Network stays `authority: network vocabulary VACANT`.
The BMCR reset ran. Probe and submit BMSR are both down, so the
doorbell is refused. The TSV window is all zero. The Apple NIC pcap
across that boot has no `0x88b5` and no source `02:00:00:00:00:01`
(only the laptop's DHCP). BMCR reset is not a UniMAC send and not a
wire frame. None of those is a NIC.

The earlier multi-boot capture `.serial-log/20260814-174803.log` paid
reset recovered (boot 7), the RX refuse (boot 6), the TX refuse
(boot 5), queue0 enable/program, link, and PHY identify. The earlier
capture `20260814-140651.log` paid FDT + `rev=6.0`. The product
capture `20260814-113438.log` (`src=dcc997cc`) paid ADR-0102/0103.
