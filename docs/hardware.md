...
`authority: network vocabulary VACANT`. The kernel maps a compiled 64 KiB
Device window at `0xFD58_0000` and, when the FDT binding matches that
window, reads `SYS_REV_CTRL` only. That is not a Pi 4 NIC claim.
