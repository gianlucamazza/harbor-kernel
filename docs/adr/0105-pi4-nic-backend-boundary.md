...
`Genet::probe`. Silicon stamp `20260816-052739.log` (`src=f0bae772`, PowerOn) has the
...
the 124-byte TSB+probe after `RBUF_CTRL` 64B+align;
packed `0x49c`, Linux `0x4a8`, and `pok` `0x4ec` are all zero. The
Apple NIC pcap has no `0x88b5`. A later host slice writes
`DMA_ARBITER_WRR` to TDMA `ARB_CTRL` before enable; that is unpaid on
