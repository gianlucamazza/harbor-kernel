...
`Genet::probe`. Silicon stamp `20260814-232303.log` boot 6 (`src=8b896ceb`, PowerOn)
...
`genet: reset recovered (idle, not a nic)`, and `VACANT`. Descriptor
ring 16 ran; the attached laptop Apple NIC received 0 frames. Those
are not completed TX/RX and not this ADR's NIC gate. No TX/RX
completion or Pi absent-device result was produced.
