...
`20260814-232303.log` boot 8) printed `tx complete len=60 (one frame,
not a nic)` (CONS posted) and `rx unavailable (timeout)` after a first
`link=down` snapshot; the laptop received 0 frames. Serial complete is
CONS, not a wire frame. The network vocabulary stays vacant.
