...
| [ADR-0100](adr/0100-device-windows.md)                               | Device windows named by index, not by physical address; `held::Set<T>` generic; store carries `window` + `va` (**accepted**); done (QEMU)                                                    |
| [ADR-0101](adr/0101-composed-driver-agent.md)                        | First composed driver-agent: `entropy` arrives in the store holding the `rng` window and reads the device before it speaks; `absent` is not `FAILED` (**accepted**); done (QEMU), HW pending |
