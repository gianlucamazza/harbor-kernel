...
| **H1 next**                  | Pi4 GENET v5 backend implementation and hardware evidence for the completed QEMU P3 edge-gateway target ([ADR-0104](adr/0104-p3-edge-network-composition.md), [ADR-0105](adr/0105-pi4-nic-backend-boundary.md), proposed design [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md)); the product prints a `genet:` FDT report, maps a compiled 64 KiB GENET window, probes `rev=6.0`, identifies PHY, prints a **probe** BMSR sample, programs Linux v5 default TX ring 0 (`TxRingSet` mask `0x1f` on TDMA `RING_CFG`, doorbell 0), WRR, RGMII OOB, UniMAC frame/station, TSB TBUF, and may print one CONS-retire TX line (`tx cons`, not a wire frame), one RX line, and one reset-recovered line; it keeps the network vocabulary vacant. Silicon evidence is in [verification](verification.md); ADR-0105/0106 stay proposed. P2 durable endpoint **done (HW)** in [ADR-0103](adr/0103-p2-el0-durable-endpoint.md) |
