...
| **H1 next**                  | Pi4 GENET v5 backend evidence for the completed QEMU P3 edge-gateway target ([ADR-0104](adr/0104-p3-edge-network-composition.md), [ADR-0105](adr/0105-pi4-nic-backend-boundary.md), design [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md), method [ADR-0107](adr/0107-genet-sequence-first-bring-up.md), link [ADR-0108](adr/0108-boot-path-link-acquisition.md)). On 2026-08-17 the six items of the ADR-0105 gate were produced on silicon — probe, link state, a bounded TX confirmed by UniMAC's counter and by an `0x88b5` frame on the wire, a bounded RX, reset/recovery, and an absent-device refusal — recorded boot by boot in [verification](verification.md#hardware-evidence-pi-4-genet-v5-bring-up-2026-08-14--2026-08-17). **Accepted 2026-08-17** on that evidence. What is paid is the backend; publication is not — the network vocabulary stays vacant on `raspi4b` and P3 stays `done (QEMU)` until the backend is bound to it. P2 durable endpoint **done (HW)** in [ADR-0103](adr/0103-p2-el0-durable-endpoint.md) |
...
| **open (product)**           | Pi4 GENET v5: backend **accepted** with hardware evidence ([ADR-0105](adr/0105-pi4-nic-backend-boundary.md)); what remains is publishing it to the network vocabulary, which needs a design ADR before code; P4 remains deferred (ADR-0049) and is a decision rather than a slice; denser composition (K5-H if slots bind); console server remains compiled-in EL1 infrastructure |
...
| [ADR-0105](adr/0105-pi4-nic-backend-boundary.md)                     | Pi 4 NIC backend boundary and evidence gate (**accepted**); the backend is paid on silicon, its publication is not                                                                             |
| [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md)                  | Pi 4 BCM2711 GENET v5 backend design (**accepted**); implemented, with hardware evidence; network vocabulary still vacant                                                                           |
| [ADR-0107](adr/0107-genet-sequence-first-bring-up.md)                | GENET bring-up moves from one-variable slices to sequence-first (**proposed**); the evidence gate is unchanged                                                                 |
| [ADR-0108](adr/0108-boot-path-link-acquisition.md)                   | Link acquisition in the boot path (**proposed**); the boot path does not reset the PHY, and a settle is bounded in milliseconds                                                |
| [ADR-0109](adr/0109-hardware-evidence-is-tracked.md)                 | Hardware evidence is tracked; the capture that carried it is not (**proposed**); `docs/evidence/` holds a derived record per cited capture, heartbeat collapsed, and `make hw-evidence` refuses a citation without one |
| [ADR-0110](adr/0110-a-model-is-consumed-or-declared.md)              | A pure model is consumed, or it names what will consume it (**proposed**); reachability says 9 of 100 `genet` pub items are unreachable from the driver, four were deleted (one of them a wrong ring wrap) and five declared design-ahead for the P3 publication |
| [ADR-0111](adr/0111-product-boot-witness.md)                         | The product leaves a witness that it booted, off the serial line (**proposed**); the media boot counter moves from `demos.rs` into the shipped image, so an empty capture stops meaning both "dead channel" and "dead board" |
| [ADR-0112](adr/0112-p3-publication-transport-boundary.md)            | Publishing a NIC backend (**proposed**); the packet ABI leaves `kernel_core::virtio` for `net`, one board-selected transport module replaces the virtio-typed runtime, and the first slice polls — the design for P3 `done (HW)` |
| [ADR-0113](adr/0113-p4-framebuffer-agent.md)                         | P4 — a framebuffer agent on a device window (**proposed**); the VideoCore mailbox stays EL1 because a request channel that allocates memory is not a window, and the QEMU evidence is stronger than the hardware's because only an emulator can be asked what is on the screen |
