...
| **open (product)**           | Pi4 GENET v5: the [ADR-0105](adr/0105-pi4-nic-backend-boundary.md) gate is met on silicon and awaits the owner's acceptance, not more code; P4 remains deferred (ADR-0049) and is a decision rather than a slice; P6 compose/audit tooling is `done (QEMU)` and never exercised on hardware; denser composition (K5-H if slots bind); console server remains compiled-in EL1 infrastructure |
