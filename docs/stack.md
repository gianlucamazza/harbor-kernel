...
| Board           | Raspberry Pi 4 Model B (BCM2711), quad-core; dual-current (product home CPU 0; K8 through steal **done (HW)**) | [`hardware.md`](hardware.md), [ADR-0015](adr/0015-multi-arch-scaffold.md), [ADR-0083](adr/0083-k8-work-stealing-first-slice.md)                                                                                                                |
...
| Cores used        | Dual-current schedule (**K8** through steal **done (HW)**: queues [ADR-0076](adr/0076-k8-per-core-queues-first-slice.md)/[0077](adr/0077-smp-shared-state-discipline.md), timer+preempt EL1/EL0 on CPU 1 [ADR-0079](adr/0079-k8-per-core-timer-preemption-first-slice.md)/[0081](adr/0081-k8-el0-on-cpu1-first-slice.md), steal [ADR-0083](adr/0083-k8-work-stealing-first-slice.md); stamps 2026-08-10). Product default home remains CPU 0; CPU 1 runs secondary idle, pinned/stolen EL1 work, and lab EL0 oracles |
...
has a first code slice — [ADR-0071](adr/0071-h3-l0-x86-qemu-first-slice.md)
(`make x86-boot-check`, status **done (QEMU-x86)**); intent and non-goals remain
in [ADR-0067](adr/0067-host-lab-second-isa-intent.md). Product `make` is still
aarch64/rpi4 only. **Product and guest images are Linux-free** (no Linux ABI or
bootloader chain in the target path). The **dev host** that runs `cargo` /
`make` / QEMU may be Linux and is **not** part of the product TCB —
...
| --------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `board-rpi4`    | **on**  | The active **product** board package. Exactly one `board-*` per image; `--no-default-features` refuses with a named `compile_error!`                                                               |
...
treats as a bug: optional tools skip _with a message_, never silently.
...
| Agent density residual                | **K5** thin done (HW); policy [ADR-0085](adr/0085-k5-density-residual-design.md): K5-S Mini first code; K5-H/B deferred                                      |
