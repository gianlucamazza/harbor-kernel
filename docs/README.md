...
| **H1 next**             | Pi4 GENET v5 backend implementation and hardware evidence for the completed QEMU P3 edge-gateway target ([ADR-0104](adr/0104-p3-edge-network-composition.md), [ADR-0105](adr/0105-pi4-nic-backend-boundary.md), proposed design [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md)); the product prints a `genet:` FDT report, probes `rev=6.0` on silicon, and after a successful probe prints PHY-identify and BMSR link lines, then programs and enables the descriptor ring (16), may print one RGMII OOB line and one UniMAC frame/station line, and may print one bounded TX report, one bounded RX report, and one reset-recovered line (a down BMSR refuses before the doorbell or RX arm; recover returns Idle); it keeps the network vocabulary vacant; a general Pi4 oracle boot baseline is now stamped in verification; P2 durable storage is an EL0 request/reply service **done (HW)** ([ADR-0103](adr/0103-p2-el0-durable-endpoint.md)) |
