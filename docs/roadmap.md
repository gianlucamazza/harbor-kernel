...
| P3  | Network agent + caps                    | **resident transport + IRQ binding integrated** — host-tested modern feature negotiation, split-queue arithmetic, bounded packet ownership, QEMU `virt` evidence for two EL1-owned split queues, `DRIVER_OK`, retained frame ownership, 32 slot IRQ bindings, and absent-device refusal ([ADR-0104](adr/0104-p3-edge-network-composition.md)); packet service, reset exercise, directional caps, and TX/RX remain open                                                                                                                                                                                                 | Network I/O only via granted caps                    | K1/K9 helpful      | H1 edge → H2 |
...
|  next | **P3 network implementation**                            | ADR-0104 names the edge-gateway composition and virtio-net target; retain the split queues across a resident EL1 service, bind IRQ/reset ownership, and implement EL0 directional capabilities. The dedicated `make qemu-virtio-check` gate currently covers queue lifecycle and absent-device refusal; TX/RX evidence is still required. | — |
