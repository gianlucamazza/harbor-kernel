...
|  next | **GENET queue-0 program**                             | After frames exist, `DmaWindows::map_cpu` translates two identity frames into the FDT DMA windows and `configure_queue0` writes queue-0 rings/descriptors. DMA stays disabled (`DmaPhase::Programmed`, not Enabled). Host-tested. QEMU has no successful probe, so no queue0 line. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
