...
| **H1 — Composition / appliance OS** | Multi-agent product you can compose and load without rebuild-only demos; early device/supervisor story      | **Paid (HW stamp 2026-08-08):** bar 1–7 + K5 thin + P2 durable + K4 budget + lifecycle residuals (serial `.serial-log/20260808-030219.log`). **Still open / residual:** P3/P4 (deferred without composition target)                                                                                 |
...
2. **Run several agents** from a product store, not a single beacon (P1; **done (HW)** — stamp 2026-08-11 loads beacon + chirp from the in-image store on silicon; the 2026-08-08 stamp had used builtin beacon+mute).
...
| --- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ------------------ | ------------ |
| P1  | Multi-agent product image beyond beacon | **done (HW)** first slice (beacon + chirp in store; Pi stamp 2026-08-11, transcript `20260811-122821.log` — both agents loaded from the in-image store and both sets of bytes reached the wire through the console endpoint, `?` and `H!`)                                                                                                                    | Product store n≥2; both run via console endpoint     | ADR-0027/0029      | H1           |
...
|     — | ~~Slot meter measured~~                                   | **Paid (QEMU)** — [ADR-0098](adr/0098-slot-meter-measured.md); `oracle-census` boots the product and reads `slots=`, measured peak **5/54**                                                                                                                                                                                                                        | —                                                               |
...
|     — | ~~Device windows~~                                        | **Paid (QEMU)** — [ADR-0100](adr/0100-device-windows.md); a window is named by **index** into a declared vocabulary, never by a physical address on the wire, so a store cannot mint memory. The mechanism ships with no window declared: every entry naming one is refused by arithmetic, seen on every oracle boot                                               | —                                                               |
|     — | ~~Composed driver-agent~~                                 | **Paid (QEMU)** — [ADR-0101](adr/0101-composed-driver-agent.md); `entropy` arrives in the store holding a window instead of being compiled in, and reads `RNG_CTRL` before it speaks, so an encoder that dropped the load changes the byte on the wire. QEMU's `raspi4b` has no RNG200, so the refusal path is what runs daily and the positive path is a Pi stamp | —                                                               |
|  next | **Services on endpoints** — what the product binds        | The vocabulary now has both its words and a sentence. What is still compiled in is the console _server_: the product mints it in `authority::assemble` and every store agent names slot 0. Composing that service the way `entropy` is composed is what P3/P4 have been waiting for                                                                                | —                                                               |
|  held | **K5-H** design (**no slot wall**)                        | The trigger now has a number instead of an opinion: measured product peak **5 of 54** slots (3 agents + 2 idle), one tenth of the ceiling. ADR-0085 §3 keeps K5-H deferred until that peak approaches it; `oracle-census` is what will say so                                                                                                                      | —                                                               |
...
- `spinner exited`). **K8 steal** **done (HW)**
  ...
  **Next:** optional K5-H design if slot wall; trigger-only K5-B **code**, K7-T,
  agent+TLB, H3 L1+, P3–P4. HW stamps for 0088/0090 optional.
