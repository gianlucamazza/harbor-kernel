...
|  next | **GENET CONS without UMAC**                           | Host: TSV window proves Linux `tx_pkts` is `0x4a8` (0xC RX/TX hole; packed trap is `0x49c`) and `pok` is `0x4ec`. DMA→UMAC handoff: clear `TBUF_64B_EN` (probe frame has no 64-byte TSB), release leftover `SYS_TBUF_FLUSH`, pulse-and-clear `UMAC_MIB_CTRL`, and put `MAX_FRAME << 16` on TDMA `FLOW_PERIOD` for ring 16. Prints `tbuf raw` then `umac tsv packed= linux= pok=`. QEMU has no successful probe. Silicon unpaid. Serial complete is not a wire frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
