...
| P3  | Network agent + caps                    | **transport probe integrated** — host-tested modern feature negotiation, split-queue arithmetic, bounded packet ownership, and a QEMU `virt` modern virtio-mmio probe with absent-device refusal ([ADR-0104](adr/0104-p3-edge-network-composition.md)); split queues, packet service, directional caps, and TX/RX remain open                                                                                                                                                                                                 | Network I/O only via granted caps                    | K1/K9 helpful      | H1 edge → H2 |
...
|  next | **P3 network implementation**                            | ADR-0104 names the edge-gateway composition and virtio-net target; implement split queues/DMA, the EL1 service, and EL0 directional capabilities. The dedicated `make qemu-virtio-check` gate currently covers transport probe and absent-device refusal; TX/RX evidence is still required. | — |
