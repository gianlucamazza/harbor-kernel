...
|  next | **GENET bounded RX**                                  | After Enabled, one RX report that refuses when BMSR is down (no consume, no vocabulary). Enabled+Up would wait for one frame. TX-complete remains unpaid until a live link. Host-tested first. QEMU has no successful probe, so no RX line. Not a NIC; ADR-0105/0106 stay proposed. | — |
