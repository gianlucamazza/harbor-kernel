...
| **H2 — Boundary OS**                | Full boundary OS: fair execution, denser agents, production isolation, multi-core, remaining platform paths | **K4** budget + EL0 + EL1 preemption **done (HW)** ([ADR-0068](adr/0068-k4-el1-preemption-second-slice.md); stamp 2026-08-09); **K5** driver-half remainder; **K7** first slice **done (HW)**; **K8** unpark **done (HW)** ([ADR-0070](adr/0070-k8-smp-first-slice.md)) + IPI wake **done (QEMU)** ([ADR-0074](adr/0074-k8-ipi-wake-second-slice.md)); per-core queues residual; P3/P4 deferred; **P2** SD power-cycle **done (HW)** ([ADR-0066](adr/0066-sd-media-durable-store.md); stamp 2026-08-09) |
...
| K8  | SMP                                                       | **done (HW)** unpark ([ADR-0070](adr/0070-k8-smp-first-slice.md): stamp 2026-08-09, `smp: core1 alive`); **done (QEMU)** IPI wake ([ADR-0074](adr/0074-k8-ipi-wake-second-slice.md): `smp: core1 ipi`); design [ADR-0048](adr/0048-k8-smp-design.md); per-core queues residual                                                                                                                                 | Unpark secondary + IPI + per-core queues                                   | 0006 → 0048 → 0070 → 0074                           |
...
| 2    | **K5** driver-half residual / K8 per-core queues     | Density / multi-core sched (IPI paid QEMU) |
