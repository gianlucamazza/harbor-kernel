...
|  next | **GENET bounded RX**                                  | After `queue0 enabled`, `RxReport::refuse` blocks UniMAC `RX_EN` and the RDMA post when BMSR is down. Enabled+Up posts one buffer and waits for CONS. Prints `genet: rx unavailable (link down)` or `rx complete len=…` or timeout. No vocabulary. Host-tested. QEMU has no successful probe, so no RX line. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
