...
| P3  | Network agent + caps                    | **transport + queue lifecycle integrated** — host-tested modern feature negotiation, split-queue arithmetic, bounded packet ownership, and QEMU `virt` evidence for two EL1-owned split queues, `DRIVER_OK`, reset/release, and absent-device refusal ([ADR-0104](adr/0104-p3-edge-network-composition.md)); persistent IRQ service, packet service, directional caps, and TX/RX remain open                                                                                                                                                                                                 | Network I/O only via granted caps                    | K1/K9 helpful      | H1 edge → H2 |
