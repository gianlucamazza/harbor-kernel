...
|  next | **GENET honest CONS + UniMAC speed**                  | Refuse a non-idle CONS before the doorbell (`implausible cons`). Complete only if CONS posted **and** Driver OWN. Then program UniMAC speed from BCM54213 AUX (host-decoded; unknown = refusal). RX complete needs a named host frame after `rx armed`. QEMU has no successful probe. Not a NIC; ADR-0105/0106 stay proposed. | — |
