...
|     — | ~~GENET compiled window + rev probe~~                 | **Paid (host/QEMU)** — `board-rpi4` maps 64 KiB at `0xFD58_0000`; `SYS_REV_CTRL` is read only when the FDT binding matches that window. QEMU never matches (no node / no DTB), so the probe line is absent there. Silicon recapture is the next stamp, not a NIC. | — |
