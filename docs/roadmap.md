...
|     — | ~~GENET compiled window + `Genet::probe`~~            | **Paid (host)** — `board-rpi4` maps 64 KiB at `0xFD58_0000`; a matching FDT binding runs `Genet::probe` (mask / stop DMA / UniMAC reset) and prints `rev=` or a bounded refusal. QEMU prints `probe unavailable (no binding)` every day. The silicon `rev=` line is HW pending. Not a NIC; ADR-0105/0106 stay proposed. | — |
