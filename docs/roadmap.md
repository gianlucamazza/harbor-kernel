...
| P3  | Network agent + caps                    | **transport + split-ring descriptor lifecycle + EL1 packet service + directional caps integrated** — host-tested modern feature negotiation, descriptor arithmetic, bounded packet ownership, wire-token validation, QEMU `virt` evidence for private RX/TX buffers, `DRIVER_OK`, TX descriptor submission/completion, copy-backed TX acceptance/completion through directional endpoints, retained frame ownership, 32 slot IRQ bindings, and absent-device refusal ([ADR-0104](adr/0104-p3-edge-network-composition.md)); deterministic peer RX, reset/recovery gate evidence, and Pi 4 NIC hardware backend evidence ([ADR-0105](adr/0105-pi4-nic-backend-boundary.md)) remain open                                                                                                                                                                                                 | Network I/O only via granted caps                    | K1/K9 helpful      | H1 edge → H2 |
...
|  next | **P3 network evidence completion**                       | ADR-0104 names the edge-gateway composition and virtio-net target; the resident EL1 service now owns split queues, bounded payload copies, restart handling, external-store packet-pool encoding, and EL0 directional capabilities. The dedicated `make qemu-virtio-check` gate covers descriptor lifecycle and absent-device refusal; deterministic peer RX and reset/recovery evidence remain required. Pi 4 backend design is tracked separately in [ADR-0105](adr/0105-pi4-nic-backend-boundary.md). | — |
