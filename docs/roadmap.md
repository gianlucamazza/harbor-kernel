...
|  next | **GENET TX on the wire**                              | Serial complete is CONS. A frame on the wire needs a raw capture on the Apple NIC (`0x88b5`) taken across the next PowerOn, not `ip -s` alone. Not a NIC until that capture. ADR-0105/0106 stay proposed. | — |
