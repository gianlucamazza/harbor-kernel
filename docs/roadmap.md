...
|     — | ~~GENET compiled window + `Genet::probe`~~            | **Paid (HW)** — last boot in `20260814-140651.log` (`src=92c889f4`): `binding ok` and `probe unavailable (Unsupported(6))`, vocab vacant. The SYS_REV_CTRL read ran; encoded major 6 is this BCM2711. The decoder now accepts the v5 encodings 6/7 (Linux remaps those to logical v5; encoded 5 is v4) and will print `rev=6.…` on the next flash. QEMU stays `probe unavailable (no binding)`. Not a NIC; ADR-0105/0106 stay proposed. | — |
|  next | **P3 Pi4 GENET backend**                              | ADR-0104 is complete at the QEMU target. Unpublished host TX/RX already lives in `genet_emulation.rs`. Backend implementation and NIC evidence remain gated by [ADR-0105](adr/0105-pi4-nic-backend-boundary.md), with the proposed design in [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md). | — |
