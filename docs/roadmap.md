...
|     — | ~~GENET compiled window + `Genet::probe`~~            | **Paid (HW)** — last boot in `20260814-140651.log` (`src=92c889f4`): `binding ok` and `probe unavailable (Unsupported(6))`, vocab vacant. The SYS_REV_CTRL read ran; encoded major 6 is this BCM2711. The decoder now accepts the v5-family encodings 5–7 (same descriptor contract) and will print `rev=6.…` on the next flash. QEMU stays `probe unavailable (no binding)`. Not a NIC; ADR-0105/0106 stay proposed. | — |
