...
|     — | ~~GENET FDT report~~                                  | **Paid (QEMU)** — `kernel_core::genet_fdt::boot_report` prints `genet:` after `discover:`; not a `discover:` fact (ADR-0072). QEMU product-boot is DTB-less (`unavailable (no dtb)`); QEMU with the Pi 4 fixture deletes the node (`unavailable (Missing)`). Not a NIC: vocabulary stays vacant; ADR-0105/0106 stay proposed. Silicon `binding ok` needs a recapture after this line. | — |
|  next | **P3 Pi4 GENET backend**                              | ADR-0104 is complete at the QEMU target: the resident EL1 service owns split queues, bounded payload copies, restart handling, external-store packet-pool encoding, and EL0 directional capabilities; `make qemu-virtio-check` covers deterministic peer RX, reset/recovery, descriptor lifecycle and absent-device refusal. The product now explains a vacant network vocabulary with a `genet:` FDT report. GENET backend implementation and NIC evidence remain gated by [ADR-0105](adr/0105-pi4-nic-backend-boundary.md), with the proposed design in [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md). | — |
