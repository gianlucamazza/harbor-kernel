...
|     — | ~~Composition vocabulary~~                                 | **Paid (QEMU)** — [ADR-0099](adr/0099-composition-vocabulary.md); `held` is declared, a failed mint leaves a hole instead of shifting every later index, `make vocabulary-sync` compares the kernel's copy against the packer's | — |
|  next | **Composable authority** — windows, services, naming      | The reason P3/P4 have no composition target is that authority is not expressible in a store: no device field ([`agentstore.rs`](../crates/kernel-core/src/agentstore.rs) writes `device: None`), one word of vocabulary, no EL0 toolchain. ADR-0099 is the first slice; windows, an endpoint service and what the product binds follow, each its own ADR | — |
