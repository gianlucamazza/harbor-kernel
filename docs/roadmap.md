...
|     — | ~~GENET UniMAC speed~~                                | **Paid (HW)** — stamp 2026-08-15, transcript `20260814-232303.log` (`src=3ee58791`, PowerOn): first BMSR `link=down`, then `tx unavailable (timeout)` / `rx unavailable (timeout)` after enable. Not `unknown speed`, not complete. Laptop Apple (`enp0s20f0u5u2`, MAC `08:6d:41:e5:db:60`) RX=0. The printed line is timeout; MDIO failure in the speed path also prints `timeout`, so this is not a written-`CMD_SPEED_*` claim. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET RGMII OOB~~                                   | **Paid (HW)** — stamp 2026-08-15, transcript `20260814-232303.log` boot 4 (`src=d53194dc`, PowerOn): `genet: rgmii oob (ext-gphy, not a nic)` after enable, then still `tx/rx unavailable (timeout)`. Laptop Apple RX=0. Mode programming ran; it did not put a frame on the wire. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET UniMAC frame~~                                | **Paid (HW)** — stamp 2026-08-15, transcript `20260814-232303.log` boot 5 (`src=6addf8f4`, PowerOn): `genet: umac init (frame, not a nic)` after RGMII OOB, then still `tx/rx unavailable (timeout)`. Laptop Apple RX=0. Max-frame and station address ran; they did not put a frame on the wire. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET descriptor ring 16~~                          | **Paid (HW)** — stamp 2026-08-15, transcript `20260814-232303.log` boot 6 (`src=8b896ceb`, PowerOn): `desc ring programmed (16)` / `enabled`, then still `tx/rx unavailable (timeout)`. Laptop Apple RX=0. Ring 16 ran; it did not put a frame on the wire. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET honest TX timeout~~                           | **Paid (HW)** — stamp 2026-08-15, transcript `20260814-232303.log` boot 7 (`src=fa00d083`, PowerOn): `tx unavailable (still owned)` (CONS posted, Device still OWNs), `rx unavailable (timeout)`. Laptop Apple RX=0. Not `mdio timeout`. DMA saw the doorbell; GENET TX did not write back OWN. Not a completed frame on the wire. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET TX CONS + CRC~~                               | **Paid (HW, serial only)** — stamp 2026-08-15, transcript `20260814-232303.log` boot 8 (`src=19b6438d`, PowerOn): `tx complete len=60 (one frame, not a nic)`, `rx unavailable (timeout)`. Laptop Apple RX=0: no `0x88b5` on the host counters. CONS posted; that is not a wire frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET TX on the wire~~                              | **Paid (negative)** — stamp 2026-08-15, serial `.serial-log/20260815-092435.log` (`src=19b6438d`, PowerOn) printed `tx complete len=60`; raw pcap `.serial-log/20260815-103422-apple-nic.pcap` on the Apple NIC across that boot has **zero** `0x88b5` and **zero** frames from `02:00:00:00:00:01`. The five captured frames are the laptop's own DHCP broadcasts. CONS complete is not a wire frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET UniMAC datapath~~                             | **Paid (HW)** — stamp 2026-08-15, transcript `20260815-092435.log` boot 2 (`src=a7891eb1`, PowerOn): `tx complete len=60` then `umac tx pkts=0 (mib, not a nic)`. Confirmed 2026-08-16, transcript `20260816-052739.log` (`src=a7891eb1`, PowerOn): same two lines. Pcap on the Apple NIC has no `0x88b5`. CONS posted; UniMAC TSV at `0x4a8` is zero. Not a wire frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
|     — | ~~GENET CONS without UMAC~~                           | **Paid (HW)** — stamp 2026-08-16, transcript `20260816-052739.log` (`src=b853980b`, PowerOn): `tbuf raw`, `tx complete len=60`, `umac tsv packed=0 linux=0 pok=0`. Pcap on the Apple NIC has no `0x88b5`. The 0xC hole is real: neither the packed trap nor Linux `tx_pkts`/`pok` moved. TBUF raw + ring-16 max-pkt + MIB release did not produce a UniMAC send. CONS posted; not a wire frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
|  next | **GENET DMA→UMAC handoff**                            | Host: product TX/RX doorbell Linux v5 default ring 0 (`DEFAULT_TX_RING`; `RING_CFG=bit0`, not `1<<16`). TBUF stays raw. TSV window stays. QEMU has no successful probe. Silicon unpaid. One variable: whether ring 16 was a DMA-only path. Not a NIC; ADR-0105/0106 stay proposed. | — |
