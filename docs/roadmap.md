...
|  next | **Composable authority** — first composed driver-agent     | ADR-0099 gave the vocabulary its first word (capabilities), [ADR-0100](adr/0100-device-windows.md) its second (device windows). What is still missing is a *sentence*: a driver-agent that arrives in a store with a window instead of being compiled in — which is what P3/P4 have been waiting for. Then services on endpoints, and what the product binds | — |
