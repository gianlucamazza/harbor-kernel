...
|  next | **GENET bounded TX**                                  | After `queue0 enabled`, `TxReport::refuse` blocks the doorbell when BMSR is down. Enabled+Up submits one frame and waits for CONS. Prints `genet: tx unavailable (link down)` or `tx complete len=…` or timeout. No RX, no vocabulary. Host-tested. QEMU has no successful probe, so no TX line. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
