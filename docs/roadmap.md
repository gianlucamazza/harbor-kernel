...
|  next | **GENET link classify**                               | After a successful PHY identify the product prints one `LinkReport` line: `genet: link=up|down (bmsr, not a nic)` or a bounded MDIO refusal. Host: `LinkReport::from_bmsr` drives `PhyLink::classify_bmsr`. Cable-down is `link=down`, not a failure. QEMU has no successful probe, so no link line. Silicon recapture unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
...
the Pi 4 path prints a `genet:` FDT report, a `rev=6.0` MMIO probe, and leaves the network vocabulary vacant;
