...
|  next | **GENET bounded TX**                                  | One TX descriptor submit/completion after Enabled. Still no vocabulary, no RX claim. Host TX path already lives in `genet_emulation.rs`. Gated by [ADR-0105](adr/0105-pi4-nic-backend-boundary.md). | — |
