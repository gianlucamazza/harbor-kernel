...
|  next | **GENET reset/recovery**                              | After the TX/RX reports, a bounded `reset()` returns the controller to Idle and prints one recovery line. No vocabulary, no NIC. TX/RX-complete remain unpaid until a live link. Host-tested first. QEMU has no successful probe, so no reset line. Not a NIC; ADR-0105/0106 stay proposed. | — |
