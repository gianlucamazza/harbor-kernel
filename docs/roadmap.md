...
|  next | **GENET honest TX timeout**                           | Split `timeout` into `mdio timeout` (MDIO during speed), `timeout` (CONS never posted), and `still owned` (CONS posted, Device still OWNs). Host-tested. QEMU has no successful probe. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
