...
|  next | **GENET UniMAC datapath**                             | Before the doorbell: `umac_cmd_datapath` (speed + `TX_EN`/`RX_EN`/`PAD_EN`/`CRC_FWD`/`NO_LEN_CHK`), `RGMII_LINK`, `TX_FLUSH` pulse, TX BD `QTAG|APPEND_CRC`. After CONS, print `umac tx pkts=` from TSV `0x4a8`. Host-tested. QEMU has no successful probe. Silicon unpaid. Serial complete is not a wire frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
