...
|  next | **GENET RGMII OOB**                                   | After Enabled, write `SYS_PORT_CTRL=EXT_GPHY` and `EXT_RGMII_OOB_CTRL` (`RGMII_MODE_EN`, clear `OOB_DISABLE`, leave `ID_MODE_DIS` clear for `rgmii-rxid`). On Enabled+Up, OR `RGMII_LINK` before `TX_EN`. Host-tested. QEMU has no successful probe. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
