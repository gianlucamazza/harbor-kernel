...
|  next | **GENET reset/recovery**                              | After the TX/RX reports, `ResetReport::refuse` blocks Idle. Enabled/Programmed calls `reset()` (stop DMA, UniMAC SW reset) and prints `genet: reset recovered (idle, not a nic)` or timeout. No vocabulary. Host-tested. QEMU has no successful probe, so no reset line. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
