...
|  next | **GENET queue-0 enable**                              | After `queue0 programmed`, `enable_queue0` writes `RING_CFG`+`CTRL` (`DmaPhase::Enabled`). Prints `genet: queue0 enabled (dma, not a nic)` or a bounded phase refusal. No TX/RX completion, no vocabulary. Host `QueueEnable` / `DmaPhase::enable` already exist. QEMU has no successful probe, so no enable line. Silicon unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
