...
|     — | ~~GENET FDT report~~                                  | **Paid (QEMU)** — `kernel_core::genet_fdt::boot_report` prints `genet:` after `discover:`; not a `discover:` fact (ADR-0072). QEMU product-boot is DTB-less (`unavailable (no dtb)`); QEMU with the Pi 4 fixture deletes the node (`unavailable (Missing)`). Not a NIC: vocabulary stays vacant; ADR-0105/0106 stay proposed. | — |
|  next | **GENET FDT silicon stamp**                           | The 2026-08-14 product transcript predates the `genet:` line, so `hw-check` against it fails until a recapture. Expected on firmware DTB: `genet: binding ok … (fdt, not probed)` and `authority: network vocabulary VACANT`. Record whatever the board prints; do not invent the node. | — |
|       | **P3 Pi4 GENET backend**                              | After the stamp. ADR-0104 is complete at the QEMU target. Unpublished host TX/RX already lives in `genet_emulation.rs`. Backend implementation and NIC evidence remain gated by [ADR-0105](adr/0105-pi4-nic-backend-boundary.md), with the proposed design in [ADR-0106](adr/0106-pi4-genet-v5-backend-design.md). | — |
...
the Pi 4 path prints a `genet:` FDT report and leaves the network vocabulary vacant;
...
**Next:** recapture the product `genet:` line on Pi 4B silicon. Then Pi4 GENET
v5 backend implementation and NIC evidence for the completed QEMU P3
edge-gateway target.
