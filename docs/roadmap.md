...
|  next | **GENET TX CONS + CRC**                               | TX complete is CONS posted (Linux reclaim; silicon `still owned` falsified OWN writeback). TX descriptors set `DMA_TX_APPEND_CRC`. Host-tested. QEMU has no successful probe. Silicon unpaid. Serial `complete` still needs laptop `0x88b5` to claim a frame. Not a NIC; ADR-0105/0106 stay proposed. | — |
