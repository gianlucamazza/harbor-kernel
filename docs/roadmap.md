...
|  next | **GENET PHY identify**                                | After a successful `Genet::probe` the product prints one `PhyIdentify` line: `genet: phy=… (id, not a nic)` or a bounded refusal (`absent id` / `stuck-high id` / `timeout` / `mode`). Host: `PhyIdentify::from_identify` drives `PhyLink::identify`. QEMU has no successful probe, so no PHY line. Silicon recapture unpaid. Not a NIC; ADR-0105/0106 stay proposed. | — |
