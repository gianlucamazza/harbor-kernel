...
link, programs and enables Linux v5 default TX ring 0, programs RGMII OOB for `rgmii-rxid`, programs UniMAC max-frame, station address, datapath (`TX_EN`/`RX_EN`/pad), and a 64-byte TSB TBUF plus `RBUF_TBUF_SIZE_CTRL=1`, and may attempt one bounded TX, one bounded RX, and one recover
