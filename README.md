...
link, programs and enables the descriptor ring (16), programs RGMII OOB for `rgmii-rxid`, programs UniMAC max-frame, station address, datapath (`TX_EN`/`RX_EN`/pad), and a raw TBUF (no 64-byte TSB), and may attempt one bounded TX, one bounded RX, and one recover
