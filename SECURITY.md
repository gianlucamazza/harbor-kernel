...
| **Today**   | Foundation + H1 depth + K4/K7-first/K8-through-steal + F-R1-P1 (+ loader lock) + **K5-S** Mini on Pi 4B; product `home_cpu` + K10 force-exit **done (HW)** ([ADR-0088](docs/adr/0088-product-home-cpu.md)/[0090](docs/adr/0090-k10-force-exit-running.md)); declared composition vocabularies + first store-granted device **done (HW)** ([ADR-0099](docs/adr/0099-composition-vocabulary.md)/[0100](docs/adr/0100-device-windows.md)/[0101](docs/adr/0101-composed-driver-agent.md)); product name bind + P2 EL0 durable endpoint **done (HW)** ([ADR-0102](docs/adr/0102-product-binds-a-name.md)/[0103](docs/adr/0103-p2-el0-durable-endpoint.md)); P3 edge-gateway implementation **done (QEMU)** ([ADR-0104](docs/adr/0104-p3-edge-network-composition.md)); K5-B design paid ([ADR-0089](docs/adr/0089-k5-b-pair-collapse-design.md)). Residuals: Pi4 backend evidence for P3 ([ADR-0105](docs/adr/0105-pi4-nic-backend-boundary.md)), P4, K5-H / K5-B **code** if trigger, optional K7-M, agent+TLB steal. See [roadmap](docs/roadmap.md) |
...
**agent+TLB steal** only if product needs auto-balance of EL0 agents.
...
**4**; concurrent tasks including idle **57** (`sched::MAX_TASKS` — ceiling
...
| Per-agent `TTBR0` + user VA window                                                                                      | Yes (M5+K7 HW)        | Kernel maps **cloned** into user root with EL0-denied AP + ASID/nG ([ADR-0014](docs/adr/0014-ttbr-split-m5.md) option C, [ADR-0050](docs/adr/0050-k7-asid-first-slice.md)); TTBR1 high-half deferred with triggers ([ADR-0084](docs/adr/0084-k7-residual-policy.md))                                                                                                                                                                         |
...
| **Capability transfer / revocation**       | **Revoke (ADR-0032, done HW):** `creator_revoke` / `revoke_held` kill both channel ends; stale CapId refused on product path (`ipc: release stale refused`). **Transfer done (HW):** EL1 (ADR-0037), EL0 self/creator (ADR-0041), peer via task-cap (ADR-0054) — endpoint caps only (ADR-0055). Residual: typed transfer / receiver consent.                             |
...
| **Creator lifecycle**                      | **Reap/restart (ADR-0033, done HW) + creator-exit cascade (ADR-0038, done HW) + force-exit Running (ADR-0090, done HW):** `supervisor_reap_blocked` / `supervisor_force_exit` (safe-point flag; victim tears down own AS). Residual: remote AS destroy; IRQ-masked infinite EL0 spin without preemption.                                                                                                                   |
...
| **Product multi-core placement**           | **Composition-visible home:** store/manifest `home_cpu` ([ADR-0088](docs/adr/0088-product-home-cpu.md), **done (HW)** — default pack pins chirp@1, beacon@0). Steal remains opt-in EL1-only; agents with a live AS mark non-stealeable. Not ambient auto-balance. See product SMP policy in [`docs/architecture.md`](docs/architecture.md).                                                                          |
