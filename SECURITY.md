...
| **SMP / ASID / TTBR1**                     | **K7** first slice **done (HW)** (ADR-0050): ASID pool + CONTEXTIDR + nG user leaves; residual TTBR1 / switch-cost measurement. **K8** unpark + IPI + queues first **done (HW)** ([ADR-0070](docs/adr/0070-k8-smp-first-slice.md)/[0074](docs/adr/0074-k8-ipi-wake-second-slice.md)/[0076](docs/adr/0076-k8-per-core-queues-first-slice.md)/[0077](docs/adr/0077-smp-shared-state-discipline.md); Pi stamp 2026-08-10, transcript `20260810-130305.log`); per-core timer + EL1 preempt on CPU 1 **done (QEMU)** ([ADR-0079](docs/adr/0079-k8-per-core-timer-preemption-first-slice.md)); residual steal / EL0-on-CPU1 / HW stamp. |
