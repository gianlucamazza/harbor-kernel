...
| **SMP / ASID / TTBR1**                     | **K7** first slice **done (HW)** (ADR-0050): ASID pool + CONTEXTIDR + nG user leaves; residual TTBR1 / switch-cost measurement. **K8** unpark core1 idle **done (QEMU)** ([ADR-0070](docs/adr/0070-k8-smp-first-slice.md); design [ADR-0048](docs/adr/0048-k8-smp-design.md)); residual HW stamp + per-core runqueue; schedulable product path remains single-core until queue work. |
