...
| Busy-loop (EL0 or EL1) and starve the system        | **Mitigated** — quantum preemption on the IRQ-return epilogue at both ELs: EL0 ([ADR-0064](docs/adr/0064-k4-el0-preemption-first-slice.md), HW Pi stamp 2026-08-09, `preempt: rotated` on silicon) and EL1 ([ADR-0068](docs/adr/0068-k4-el1-preemption-second-slice.md), HW Pi stamp 2026-08-09, `preempt-el1: rotated` on silicon, transcript `20260809-151021.log`). Device IRQ handlers still never switch; masked regions are the non-preemptible sections by construction |
...
| **SMP / ASID / TTBR1**                     | **K7** first slice **done (HW)** (ADR-0050): ASID pool + CONTEXTIDR + nG user leaves; residual TTBR1 / switch-cost measurement. **K8** unpark + IPI + queues first **done (HW)** ([ADR-0070](docs/adr/0070-k8-smp-first-slice.md)/[0074](docs/adr/0074-k8-ipi-wake-second-slice.md)/[0076](docs/adr/0076-k8-per-core-queues-first-slice.md)/[0077](docs/adr/0077-smp-shared-state-discipline.md); Pi stamp 2026-08-10, transcript `20260810-130305.log`); residual steal / per-core preempt / EL0-on-CPU1. |
