...
| Gate | Script under `scripts/check/` or `scripts/boot/` → `Makefile` → README `make check` line stays in sync (`doc-claims`) → **a row in `docs/verification.md` §The layers**, which claims to list every layer and is checked by nobody |
